...
端对端测试，也就是模拟用户行为进行测试。例如，测试点击某个按钮后，是否如预期一样弹出了一个框，或者跳转到另外一个页面；提交了一个form表单后，是否弹框提示了正确的信息。由于它是模拟用户行为的一种测试，所以往往更好使，测试的效率会低于单元测试，也正因如此，写测试代码，更多时候会倾向于单元测试而非端对端测试。尽管如此，作为测试手段的丰富及补充，开发者仍需要了解如何进行端对端测试，并在合适的场景下使用端对端测试。

下面，将介绍如何使用Protractor搭建端对端的测试环境及写一个简单的例子。

Protractor简介

Protractor是一个专门为angular设计的端对端测试框架。Protractor提供了一系列的api，帮助开发写端对端的测试用例，在运行时，将启动一个真实的浏览器进行测试。只要你掌握jasmine的基本语法，再学习Protractor提供的基础api，就可以很方便地编写端对端的测试用例。
...
接着，请更新webdriver-manager到最新版本。

webdriver-manager可以帮助protractor在运行时初始化一个Selenium端后台。
...
要运行上面的测试用例，首先需启动webdriver-manager

```
webdriver-manager start
```

接着，执行protractor命令，参数为项目的protractor配置文件
...
在本章中，先引入介绍了单元测试、端对端测试的基本概念及重要性。然后介绍了jasmine及karma的基本使用方式。最后，结合angular2的实际测试场景，介绍了相关组件的测试方法及实践。阅读完本章之后，希望你可以掌握编写、管理测试用例的有效方法。本章仅仅是测试的基础入门，更多的测试技巧，还需要在测试自己项目时因地制宜，逐步摸索。通过测试，项目将具备更好的质量及更好的开发效率，单元测试技术，是一项不可或缺的技术。在前面的几个章节中，已经介绍了angular相关的基础知识。接下来，将通过一个系统的demo，阐述angular相关的最佳实践