...
3. 良好的单元测试，也是天然的软件文档
4. 单元测试帮助我们从另一个角度审查所开发的代码，一个项目，如果很难写单元测试代码，往往也意味着项目本身的代码可测性不好，解藕程度不够。
...
Jasmine是一个行为驱动的测试开发框架，可以用来测试任意的Javascript代码。也就是说，无论是Web项目，还是Node后台项目，都可以用Jasmine来写测试代码。测试代码，无非就是通过代码指定一系列输入，然后再判断输出是否满足预期。Jasmine提供灵活强大的语法，让这些代码的编写更加简单。Jasmine也是Angular团队推荐的一个测试框架，angular2甚至基于jasmine扩展了一套测试语法，集成在@angular/core包中。
...
这段示例代码虽然非常简单，但展示了用jasmine写测试的关键语法：
...
可以简单验证下执行出错的情形，把例子中的`a=true`改成`a=false`，此时再执行，显示的出错页面如下所示： ![QQ20160720-0](../_images/chapter2-8/QQ20160720-0.png)
...
Karma是一个基于Node.js 的 JavaScript 测试执行过程管理工具(Test Runner)，由是Google Angular团队出品。

 它主要带来了这些强大的特性：

- 在真实浏览器上测试。测试的一个目标，就是关注代码在不同浏览器的表现，检查是否有兼容性问题。Karma可以同时启动chrome、safari等不同的浏览器跑测试代码，并在一个浏览器窗口显示各个浏览器的执行结果。
- 当代码更新时，自动触发测试。在实际开发中，如果被测代码或者测试代码发生了变更，就能自动触发测试，这样代码的执行结果能马上得到验证，而且不用在多个编辑区、窗口之间切换。

对于几个测试用例的小demo工程，karma也许显得大材小用，但随着项目规模逐渐变大，一款值得信赖，可以很方便的管理、执行用例，输出报表的工具，将为你解决很多测试代码之外的烦心之事。
...
## 8.2.3 karma+jasmine简单例子

为了使读者能快速理解、结合karma＋jasmine进行测试，这里将展示一个简单的例子
...
将启动karma的配置生成器，命令行窗口依次询问要采用的配置，我们一路按回车键，使用默认配置即可，最终将在本地生成karma.conf.js。界面如下图所示
...
这是一个简单的配置文件，用于通过npm管理自己的项目，其中，`"test": "karma start karma.conf.js"`表示为｀karma start karma.conf.js｀命令创建了一个别名`npm test`，运行 `npm test`等于运行｀karma start karma.conf.js｀

然后执行`npm install`安装package.json上安装的依赖包

##### 写测试用例

在根目录创建一个src.js文件，包含了一个简单的函数，验证参数是否为string
...
在karma.conf加入测试及被测代码的路径，这里可以用通配符的方式，新加入配置如下：
...
`files: ['*.js']:` 指定了测试和被测代码的路径

##### 运行测试用例

最后，执行`npm test`

命令行窗口将出现以下信息，在最后一行，可以看到成功执行了一个用例

![QQ20160707-0](../_images/chapter2-8/QQ20160707-0.png)

同时，会自动打开一个chrome浏览器，显示以下内容
...
在前文中，已经提到，angular开发团队，从设计angularjs1.x到angular2，都特别注意这个框架的可测性。在angular2，组件化、及使用ECMAScript 2015/2016特性进行编写，使其单元测试更加便捷，简单。对angular2的程序进行单元测试，基本思路是：

1. 首先，angular app下的每个组件，都是一个个js class（export class ClassName {xxx})。你可以像测试普通js代码（或者更准确地说，普通的js class）一样，测试class中的逻辑。
2. 其次，为了更好的测试compoent中的交互、展示逻辑，也许你需要学习使用`TestComponentBuilder`,为例更好的测试servcie，你也许需要学习如何使用`addProviders`注入测试代码中需要引用的service，使用`async`api验证耗时的异步请求，学习如何mock service。
3. 最后，根据项目前端打包工具的不同，也许需要学习如何将webpack，system.js和karma整合，构建一套完整、可持续维护的测试工程。
...
由于angular依赖注入的特性，在angular的测试代码中，与其它的jasmine测试代码一个很大的区别，就是需要在用例中注入所引用的组件。下面展示了一个简单的依赖注入的例子，在测试用例中，需要引用angular的APP_ID这个组件，因此，需要在用例代码中，使用inject([APP_ID], (id) => {})将APP_ID注入到用例代码中，且指定了别名为id
...
一个compoent，其逻辑都是封装在class中，因此，测试一个简单的component，跟测试普通的javascript class没什么不同。让我们从最简单的开始吧！假设有comonent如下：
...
```
可以看到，跟测试普通的js class并无二致，这时候，如果class中增加了函数或者变变量，也是同样的测试方式，例如，假设新增了getHelloStr()函数，那么测试的代码可能是expect(this.app.getHelloStr()).toBe('xxxx')［todo：展示一个具体的例子，被测的函数return `Hello ${this.name}`;之类］
...
顾名思义，TestComponentBuilder的存在，是为为了帮助开发者测试component，构建用于测试component的上下文环境。其中，api`createAsync`用于初始化被测的组件，并创建ComponentFixture对象，这个对象可以认为是被测组件的上下文环境，可以通过它获取初始化好的组件，做一些dom元素的检测、交互等。

例如下面的例子中，将初始化一个GreetingComponent组件，然后将其对应的执行上下文赋给fixture这个变量。获取该变量后，常见的操作包括：

- fixture.detectChanges()[TODO:不知道怎么描述其作用，反正就是都要执行这一句，有点类似angular1里面的apply，触发js变量及模版元素值的变化渲染]
- fixture.debugElement.nativeElement：获取组件对应的dom元素，之后可以通过`querySelector`等操作元素对象的方式，做进一步处理。
- fixture.debugElement.componentInstance：获取组件对应的js实例，例如对应示例存在内部变量greeting时，则可以通过`fixture.debugElement.componentInstance.greeting`访问及修改该变量。
...
在测试component的时候，往往每个用例都需要注入TestComponentBuilder，可以用如下的方式，统一注入，而不是在每个
...
获取并点击元素：cfixture.debugElement.nativeElement.querySelector('button').click()，如例子所示，表示获取组件dom元素，并在dom元素找到tag为button的子元素，并触发click事件。

替换模版`overrideTemplate`:可以通过这个api，在初始化组件时，根据测试需要替换为其它模版。当组件模版特别大时，而测试又只希望关注其中一部分模版逻辑时，会派上用场。[TODO:需要自定义TestComponent]

［todo：要讲fakeAsync吗？一个例子，有一个按钮，点击之后2秒才会有对应的响应，可以通过这个等待2秒］
...
［todo：增加例子］service的测试，也是比较简单的，和测试普通的js class相比，唯一的不同，是因为angular2依赖注入的特性，因此需要注入测试时依赖的组件。假设要测试的service名为`UserService`,包含了一个函数`isAdmin(username)`。那么，对应的测试代码可以为
...
对于一些耗时的请求，例如http请求，也许我们的关注点是请求能否在预期的时间内返回响应。因此，我们可以加个请求等待时间，如果测试用例没有在指定时间返回响应，那么将会被判定为测试失败。以下为代码示例。通过 async(function,time)方法将测试用例包起来，其中time参数为等待的时间，例如下面的例子表示，如果用例没有在3000毫秒内执行结束，则判定为测试失败。
...
除此之外，还有另外一个方式，就是使用`fakeAsync`,用法类似，也是用这个函数把测试用例包起来，不同的地方在于，这个方法不用指定超时的时间，而是在测试用例中通过tick函数，指定要等待的时间，例如下面的例子tick(500)表示，在上面的测试用例执行之后，等待500毫秒，再继续执行下面的逻辑
...
在单元测试中，经常会遇到的一个问题，就是mock。mock就是构造一个虚拟对象或者数据，来代替真实的代码，以方便对测试代码中真正关注的对象进行测试，例如，`UserService`中调用了`LoginService`，以验证用户的登陆态，而在`UserService`的单元测试中，关心的是`UserService`本身的逻辑，因此，其调用的其他`Service`,可以通过mock的方式，使其返回固定、预期的值，方法如下：
...
如此一来，在被测代码中，当需要用到LoginService.login方法时，调用的，均是mock service，而不是原先的被测代码。
...
由于我们的demo采用了webpack做打包，因此需要结合webpack配置来修改karma.config.js。

下面展示了修改后的karma.config.js文件。主要是告诉karma需要引用webpack进行打包。其中，./webpack.config指定的是项目本身的webpack配置。karma-shim.js是要新创建的文件，指定了angular测试配置以及编译测试代码的配置
...
测试代码，是和被测代码放在同个层级目录，还是创建一个独立的test目录，这是个见仁见智的问题。但是，有一点是肯定没错的，那就是尽量保持和被测组件相同的名字前缀，然后加上spec或者test等字符串以标示其为测试代码。例如，假设被测的代码文件名为contact-list.ts，则对应的测试用例文件名可疑为contact-list.spec.ts。
