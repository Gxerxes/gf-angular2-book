...
`[( )]`是Angular提供的双向绑定语法糖，`ngModel`辅助实现双向绑定的指令。Input的值发生变更时，可自动赋值至`contact.name`，`contact.name`的值被组件类改变时，亦可实时更新Input的值，关于更多双向绑定实现细节，后续模板章节会详细展开。

数据绑定负责数据的传递与展示，针对数据的显示格式，Angular提供了管道功能，使用竖线`|`来表示，如：
...
除此之外，Angular还有很多的强大语法特性，如上组件封装的自定义标签，如`<contact></contact>`，此外还提供了一套强大的”指令“机制，来简化一些特定的交互场景，如样式处理，数据遍历，以及表单处理等，关于指令下面继续探讨，而更详细的模板语法将会在第三章一一罗列。
...
属性指令用以改变元素的外观或是行为，属性指令使用起来跟普通的HTML元素属性非常相似，如`ngStyle`指令，可以动态计算样式值。

```
<span [ngStyle]="setStyles()">{{ contact.name }}</span>
```

span标签的样式由`setStyles`函数计算得出，`setStyles()`返回的是一个样式对象，如下所示：
...
指令最具吸引力的地方在于支持自定义指令，最大限度地实现了UI层面的逻辑复用。第四章指令章节将会带大家进入Angular丰富的指令世界。
