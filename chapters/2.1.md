...
## 架构总览
一个完整的Angular应用主要由6个核心组成部分构成，分别是：组件、模板、指令、服务、注入、路由。这些组成部分各司其职，而又紧密协作，组成一个完整有序的Angular应用。各部分的关系总结如图2.1-1所示。

![](https://raw.githubusercontent.com/gf-rd/gf-angular2-book/master/_images/chapter2-1/overview.png)

图 2.1-1 Angular构架

与用户直接交互的是模板视图，模板视图是组成组件的要素之一，另一要素是组件类，维护组件的数据模型及功能逻辑，二者之间有着双向的数据流动。指令和服务是独立的功能模块，均为组件提供功能扩展，其中指令扩展模板的语法，而服务通过注入机制被引入到组件类中，扩展组件类的功能逻辑。最后组件的创建和销毁是通过路由配置控制。

在Angular应用接收用户指令，加工处理后输出相应视图的过程中，组件始终处于这个交互的出入口，这正是Angular框架基于组件设计的体现。
...
`@Component`里头的`providers`是注入`LoggerService`的关键步骤，这样组件类里便可以引入LoggerService类型的对象，并且无须显式初始化。如在构造函数里定义的`logger`变量，组件在初始化时候会为`logger`赋值为`LoggerService`的”新“实例。不错，服务的每一次注入（也就是使用`providers`声明），Angular都会为其创建新的实例，这个特性通常结合注入树发挥作用，还记得组件是以组件树的形态组织，当服务注入到某个组件时，其所有的子组件均可复用这个实例，也就是说服务在组件及其子组件里默认是以单例模式运行，除非某个子组件再次注入（即通过`providers`引入），如图所示。

![](https://raw.githubusercontent.com/gf-rd/gf-angular2-book/master/_images/chapter2-1/injector.png)

这种灵活注入方法可以适应多变的应用情景，既可配置全局单例服务（在根组件注入即可），亦可按需注入不同配置的服务实例，彼此数据状态不会交叉影响。更多关于注入的细节将会在依赖注入章节继续讲解。
...
原来的组件树中多了一个路由组件（Angular对应的组件为`router-outlet`），类似于”插座“功能，根据当前的URL路径，匹配插入对应的组件节点。这就是Angular路由最基本的功能，实现主体内容（页面）的刷新。

路由组件还支持多重嵌套，实现子路由功能。如通话记录需要新增筛选tab功能，可选全部及未接来电。
...
注意当path值为空时，表示默认路由，所以当路由至`http://www.abc.com/record`时，显示的是`AllRecordsComponent`组件视图，修改通话记录组件结构如下：

![](https://raw.githubusercontent.com/gf-rd/gf-angular2-book/master/_images/chapter2-1/sub-router.png)

路由还支持路径参数，如`http://www.abc.com/list/123`，`123`为联系人ID，实现类似Restfule风格的路由系统。另外路由组件除了支持嵌套使用，同一层节点还可以放置多个路由组件，实现从属路由功能，更多的路由功能会在路由章节一一剖析。
