...
数据绑定的核心是数据的动态更新，Angular内部维护着一套变化监测机制，监控来自Model和View的数据变动，变动触发是通过zone.js实现，组件章节将会详细讲述。

![](https://yakovfain.files.wordpress.com/2016/02/ch1_angular_component.png?w=760)


组件是Angular里的核心部分，组件的不断创建和销毁驱动着Angular应用运作。但组件并不能独立运作，其行为的执行离不开下面这些组成部分：
...
### 模板
在Angular里模板并不是独立的组成部分，而是组件的一个重要组成要素，二者有着密切的数据传递，组件在metadata里通过指定`template`和`templateUrl`与模块取得关联。Angular模板基于HTML，纯粹的HTML即可以完成Angular视图的渲染，但这不是Angular模板的全部，Angular基于HTML的语法可扩展出灵活丰富的自定义标签或自定义属性。如下通讯录列表页示例代码：
...
各个组成部分以组件作为桥梁关联起来，所以本书的第二部分将以“组件”作为深入讲解的开篇，按照与组件的关联程度，分别展开讲解模板、指令、服务、依赖注入以及路由。另外在最后安排一章“测试”，详细讲解在Angular框架下如何进行测试工作，保证项目质量。
