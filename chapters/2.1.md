...
## 架构总览
在Angular概述章节里提到Angular的的6个核心组成部分：组件、模板、指令、服务、注入、路由。这些组成部分各司其职，而又紧密协作，组成一个完整有序的Angular应用。各部分的关系总结如图2.1-1所示。
...
但Angular模板远不止于此，Angular为模板定制出一套强大的语法体系，涉及内容颇多，这也是将模板单独列出的原因。数据绑定是模板最基本的特性，除了前述提到的属性绑定和事件绑定，插值在Angular模板也是非常常用，如下所示：
...
已经提到过的三种数据绑定语法的数据流动都是单向的，在某些场景下（如表单），需要双向的数据流动支持完成功能开发，不过Angular并没有提供原生的双向数据绑定功能，但可结合属性绑定和事件绑定实现类似的功能，如：
...
### 路由
Angular作为单页应用框架，前端路由功能必不可少。在Angular里，路由的目标是建立URL路径和组件之间的关联关系，根据不同的URL路径匹配相应的组件，所以一个简单的路由配置大概可能是这样：
...
import { IndexComponent } from 'xxx';
import { ContactListComponent } from 'xxx';
...
`path`为空时，需要创建IndexComponent并渲染其视图；而`path`为`list`时（如`http://www.abc.com/list`），则需要初始化ContactListComponent组件。那么问题来了，创建的这些组件的放在组件树的哪个位置呢？Angular路由提供了`router-outlet`组件作为路由控制的容器。
