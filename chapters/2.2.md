...
### 2.1.2 组件化的标准
&emsp;&emsp;W3C 对 Web 组件提出了 WebComponent 的标准，通过标准化的非侵入方式封装组件，每个组件包含自己的 html、css、js 代码，并且不会对页面上其他代码产生影响。目前标准还没有定稿，在 [w3c](https://www.w3.org/standards/techs/components#w3c_all) 的官网可以看到标准的最新状态。

&emsp;&emsp;Web components 由一些新的技术构成，提供了浏览器原生的 UI 组件标准。因为是原生的，所以不需要引入任何外部的依赖。要使用一个已有的 Web Component，只需要添加一句导入的声明：`<link rel="import" href="example.html">`。

&emsp;&emsp;标准包括四个重要的概念：
...
&emsp;&emsp;这里通过“定义一个 Hello 组件”的简单例子，去说明 WebComponent 标准的四个概念：在页面上创建一个显示“Hello Web Component!”的字样，并给它设置为红色。
...
&emsp;&emsp;假设 Hello 组件的代码片段保存在某处的`hello.html`中，那么在页面中可以通过 HTML 导入的方式引入，最终可以在页面上通过自定义的 HTML 标记 hello 去使用它：
...
&emsp;&emsp;`hello.html`就像一个普通的 HTML 文档，这意味着可以像普通的 HTML 文档中那样引入其他脚本或样式表，比方说希望在 Hello 组件中引入 bootstrap 的样式库，那么就可以在 hello.html 开头加上`<link rel="stylesheet" href="bootstrap.css">`。这个特性使得开发者能够将多个文档（组件）封装成一个文档（组件）提供给他人使用。关键的一点是，通过 HTML 导入的特性，使得异步加载模块、执行模块变得更加容易。

&emsp;&emsp;最终`<hello></hello>`在页面上以 Shadow DOM 的方式渲染出来：
...
&emsp;&emsp;浏览器原生的 Shadow DOM 的特性正是实现了 WebComponent 组件之间互不干扰的需求，通过创建一个 shadow root 子树，将该组件与文档流中的其他 DOM 树隔离开来，彻底解决了样式冲突、全局变量污染等问题。
...
&emsp;&emsp;除此之外，Angular 支持**模板、自定义标签、异步加载组件**，Angular 的组件是**自描述**的——组件可以和宿主元素交互，组件知道如何及何时渲染自己，组件可配置注入服务，组件有明确的 Input&Output 定义（注入与服务是后续章节的内容，宿主元素、Input和Output在本章有讲述）。这使得所有 Angular 的组件都可以独立存在，这意味着任何 Angular 组件都可以通过 bootstrap 来启动，也可以被路由加载，或者在其他组件中使用，这极大地提高了组件的复用性。

&emsp;&emsp;在上一章，读者已经接触了 Angular 组件的一些概念，知道了 Angular 应用由一系列大大小小的松耦合的组件构成了一棵完整的应用组件树，以及了解了组件间的数据流动、变化监测等概念。在本章接下来的内容会结合演示代码以及 Demo 对上述概念进行详细讲解。

## 2.2 组件基础

### 2.2.1 如何创建组件
&emsp;&emsp;学习 Angular 的组件，首先要知道怎么创建它，创建组件很简单，Angular 提供了很方便的方法。此处通过创建一个显示名称和电话的联系人卡片的例子，说明组件的创建方法。

&emsp;&emsp;创建 Angular 组件的方法有三步：
...
&emsp;&emsp;以上便创建了一个最简单的 Angular 组件，要使用这个组件的话，需要在 HTML 中添加 contact-item 这个自定义标签，然后 Angular 便会在此标签中插入一个 ContactItemComponent 的实例对应的模板。
...
### 2.2.2 组件基础构成
&emsp;&emsp;刚才学习了最简单的组件创建，从那段很简单的代码当中，能看到构成组件的基础。
...
3. 模板：每个组件都会关联一个模板，这个模板最终会渲染到页面上，页面的这个 DOM 元素就是此组件实例的宿主元素
4. 组件类：组件实际上也是一个普通的类

&emsp;&emsp;其中组件的元数据并不只 selector、template，完整的元数据在本章节中也会分别讲解。

#### 组件修饰器

&emsp;&emsp;`@Component`是 TypeScript 的语法，它是一个修饰器，任何一个 Angular 的组件类都会用这个修饰器修饰，如果移除了这个修饰器这个组件就没有任何意义了，它将不再是 Angular 的组件。那么这个修饰器是什么？有什么作用？浏览器如何解释它？
...
&emsp;&emsp;可以看到，Angular 的`@Component`会被转换成一个`__decorate`方法，将元数据的定义通过`core_1.Component`传入，将`ContactItemComponent`这个类“修饰”起来，使得`ContactItemComponent`具有 Angular 组件定义的一切属性。Angular 提供的`@`操作符来实现修饰组件，并没有什么特别之处，可以理解为对组件封装的语法糖，方便开发者编写 Angular 的组件。

#### 组件元数据
&emsp;&emsp;在`ContactItemComponent`这个组件中的 @Component 修饰器部分，使用到了大部分组件需要的元数据——用于定位元素的`selector`、用于定义组件宿主元素模板的`template`，template 是用于定义内联的模板的，正如读者看到的，Angular 还提供了 tempalteUrl，可以引用外链模板。除此之外，一般组件都会有自己定义的样式，Angular 提供了 styles/styleUrls 来定义内联样式或引用外链样式。
...
&emsp;&emsp;selector 的命名方式建议使用“烤肉串式”命名，即小写并以`-`分隔。例如`contact-app`、`hello-world`。

&emsp;&emsp;在`ContactItemComponent`中，它的`selector`设置为`contact-item`，那么 Angular 便会通过查找 DOM 上的一个 contact-item 元素，从而将**组件的内容**显示在页面上。那么**组件的内容**又是怎么指定的？这就要用到 template/templateUrl 。
...
> 
> 注：本书中的某些代码例子，使用 template 仅为了方便演示。

&emsp;&emsp;在本书的例子中，每个组件都使用了外链模板，比如在`list-li.ts`中，使用了 templateUrl，为`ListChildrenComponent`指定了一个`list-li.html`模板，它的内容如下：
...
&emsp;&emsp;然后，Angular 便在 DOM 上搜索`<list-li></list-li>`这个位置，将`list-li.html`的内容都渲染到 DOM 上。这就是 selector 以及 template/templateUrl 的作用。
...
&emsp;&emsp;并通过`npm install sass-loader`，便可以在代码中使用了，上述的 styleUrls 引用的样式可以改成：
...
#### 模板
&emsp;&emsp;上面的元数据也说明了 template/templateUrl，每个组件都必须设置一个模板，Angular 才能将组件渲染到 DOM 上，而这个 DOM 元素就叫做**宿主元素**。上文也提到组件是自描述的，其中一点便是组件可以与宿主元素交互：
...
##### 显示数据
&emsp;&emsp;在模板中，可以使用`{{}}`语法来显示组件的数据，这和 Angular1.x 的版本是一致的。比方说在 ContactItemComponent 组件中，把“张三”和电话都通过成员变量来显示。
...
&emsp;&emsp;这种语法`{{}}`在 Angular 中称为**插值**，关于插值的更多内容，请参阅**模板**章节。
...
&emsp;&emsp;在上面的例子中，输入框里默认显示“张三”，并绑定了`name`属性。当输出框的内容发生改变时，Angular 的双向绑定机制便会将输入框的内容同步更新到`name`属性，并且会同步显示到`<p>{{name}}</p>`上。

&emsp;&emsp;关于双向数据绑定的语法更多细节，请参阅**模板**章节。

##### 监听宿主元素事件
&emsp;&emsp;Angular 提供了非常方便的事件绑定的方式，读者可以在本书 Demo 中的`contact-list.html`看到`(click)`这样的用法，`()`是 Angular 提供的事件绑定语法糖，通过`(eventName)`的方式可以轻易地响应UI事件。

&emsp;&emsp;contact-list.html 中事件绑定的代码：
...
&emsp;&emsp;Angular 提供了**directives**这个元数据，来引入其他指令或组件，比方说在本书 Demo 的`contact-list.ts`文件中，引入了一组依赖：
...
&emsp;&emsp;这里使用了 **directives** 这个元数据——指定可以在模版中使用的指令/组件列表。由于组件继承自指令，所以同样也能使用 direcitves 引入组件。除了 Angular 默认提供的指令，即 ng 开头的如 ngFor 等不需要显式指定外，自定义的指令或组件必须明确指定，否则在组件中无法正确使用它们。
...
&emsp;&emsp;在本书 Demo 的入口组件 `ContactApp` 中，有着这么一段代码：通过 viewProviders 元数据注入了 HTTP_PROVIDERS 和自定义的 `ContactService` 服务。`ContactService` 是维护联系人数据的主服务，提供了联系人的相关操作。关于服务的更多内容请参考**服务**章节。
...
### 2.3.1 @Input & @Output
本节的组件交互中，需要涉及到的输入（@Input）或输出（@Output）属性，下面将详细介绍相关知识。
...
在理解了输入输出属性的相关知识之后，下面将详细介绍组件间的数据交互。
...
在父组件中获取子组件成员变量和方法的权限相对的便捷一些，但是，反过来在子组件中获取父组件的实例就相对麻烦些，可以通过把父组件的实例添加到注入器的容器里，这样就能通过依赖注入来找到父组件的实例，从而获取到父组件实例对象，但是这里不再继续展开，具体请参考后面的依赖注入章节。

## 2.4 自定义内容

&emsp;&emsp;自定义内容有时被认为是一个高级的功能，使用自定义内容能很好地扩充组件的功能，特别是在需要创建团队、项目之间共享的公用的 HTML 代码片段的时候。

> 接触过 Angular1.x 的读者，相信对自定义内容不会陌生，它和 Angular1.x 中指令的`transclude`属性非常类似。

&emsp;&emsp;自定义内容通常用来创建可复用的组件，典型的例子是模态对话框或导航栏。想像一下，在开发 Web 应用的时候，模态对话框和导航栏是使用非常频繁的功能，自定义内容的特性正是提供了一种复用的方式，使得复用的组件具有样式完全一致的，但内容可自定义的组件。
...
&emsp;&emsp;`ExampleComponent`的代码如下，在模板中使用到`<ng-content>`标签，这个标签就是用来”渲染自定义内容“的。注意到在`<ng-content>`中有个`select="header"`，用于匹配内容，接下来会说明它的作用。
...
&emsp;&emsp;定义好`ExampleComponent`后，再定义一个简单的根组件去使用它：
...
&emsp;&emsp;最后组件的 DOM 树会被 Angular 处理成以下：
...
&emsp;&emsp;注意到在 example-content 标签之间的内容，也就是`<header>Header content</header>`，被填充到了 ng-content 中去了，而 `ExampleContent` 模板中的其他元素没有受到影响。那么自定义的内容是如何匹配显示的呢？是否可以有多个自定义内容？答案是可以的，上文也提及到了`select="header"`这个属性，这是一个选择器，与 css 的选择器是一致的，它表示匹配 example-content 标签之间的第一个 header 标签，并将其填充 到相应的 ng-content 中：

&emsp;&emsp;下面修改`ExampleContent`的代码，通过标签选择器、类选择器、ID选择器，来指定多个 ng-content：
...
&emsp;&emsp;使用过 Angular1.x 的读者应该知道，Angualr1.x 中有构造函数，$watch 方法和 $destroy 事件可以尝试挂接控制器中生命周期的各个时间点。在 Angular 中，生命周期的含义被定义得更加明确。当组件实现一些生命周期勾子的回调，那么在变化检测的时候就会在特定的时间点被触发。

&emsp;&emsp;组件的生命周期由 Angular 内部管理——从组件的创建、渲染，到子组件的创建和渲染，到检测数据绑定的变化，到组件从 DOM 移除。Angular 都提供了一系列的生命周期勾子，可以很方便地在这些事件发生的时间点去添加自己的代码。

### 生命周期勾子
&emsp;&emsp;指令或组件实例都有一套生命周期，表现在 Angular 对它们的创建、更新和销毁。

&emsp;&emsp;开发者可以实现一个或者多个“生命周期勾子（接口）”，从而在生命周期的各阶段作出适当的处理。这些勾子接口包含在 @angular/core 中。每个接口都有一个名为"ng"+接口名的方法，例如 OnInit 接口有一个叫 ngOnInit 的勾子方法，可以这样去实现它:
...
&emsp;&emsp;以下是组件的所有生命周期接口及对应的勾子方法，Angular 会按以下的顺序依次调用勾子方法：
...
&emsp;&emsp;除此之外，有的组件还提供了自己特有的生命周期勾子。例如路由有 routerOnActivate 勾子。

#### OnChanges
&emsp;&emsp;用于及时响应 Angular 设置数据绑定输入的属性。该方法接收一个 changes 对象，包含当前值和变化前的值。在ngOnInit 之前，或者当数据绑定输入属性的值发生变化时会触发。

&emsp;&emsp;在 Angular1.x 中，如果想要监听数据的变化，需要设置$scope.$watch，然后在每次 digest 循环里判断数据是否有改变。在 Angular 中，ngOnChanges勾子把这个过程变得简单。只要在组件里定义了 ngOnChanges 方法，在输入数据发生变化时该方法就会被自动调用。

&emsp;&emsp;需要注意的是，ngOnChanges 当且仅当组件输入数据变化时被调用，“输入数据”指的是通过`@Input`修饰器显式指定的那些数据。

#### OnInit
&emsp;&emsp;用于在 Angular 初始化数据绑定输入属性之后初始化组件/指令。该勾子方法会在第一次 ngOnChanges 之后被调用。
...
&emsp;&emsp;在 Angular 的组件中，经常会使用 ngOnInit 获取数据，为什么不在组件构造函数中获取数据呢？因为构造函数做的事，应该是尽可能简单的，例如初始化局部变量这种简单的事情。这对于有经验的开发人员来说，已经是一种共识。另外，这对于 Angular 的自动化测试也有非常重要的作用，因为编写测试代码的开发人员，不需要担心一个新的组件在测试或者创建之前会尝试连接远程服务器获取数据。

#### DoCheck
&emsp;&emsp;用于变化检测，该勾子方法会在每次变化检测发生时被调用。
...
> 注意，DoCheck 和 OnChanges 不能同时使用

#### AfterContentInit
&emsp;&emsp;在组件使用了`<ng-content>`定义内定的情况下，Angular 会在将外部内容放到视图后调用 ngAfterContentInit。

#### AfterContentChecked
&emsp;&emsp;在组件使用了`<ng-content>`定义内定的情况下，Angular 会在检测到外部内容的绑定或者每次变化的时候调用 ngAfterContentChecked。
...
#### OnDestroy
&emsp;&emsp;在 Angular 销毁指令/组件之前触发。建议把组件成员变量等东西的清理逻辑放到 ngOnDestroy 中，它是在指令/组件销毁之前必定运行的逻辑。在这个时候可以通知另一部分关心的程序，这个组将将要被销毁。
...
## 2.6 变化监测
在`Angular`中提供了数据绑定的功能（在模板章节将会详细介绍），所谓数据绑定即将组件model数据和页面DOM元素关联起来，当数据发生变化或者更改后，Angular能够检测到这些变化或者更改，并对其所绑定的DOM元素进行相应的更新，反之亦然。本节将重点讲解`Angular`的变化监测，涉及到的知识点大致如下图所示：

![](./../_images/chapters2-2/change-detection.jpg)

图 2.6-1

如上图所示，当异步事件发生导致组件中数据的变化，`Angular`并不会去捕捉对象的变动，它采用的是以适当的时机去检验对象的值是否被改动，这个时机由`NgZone`去掌控，它获取到了整个应用的执行上下文，能够对相关的异步事件发生、完成或者异常等进行捕获，从而触发`Angular`的变化监测执行，下面将详细介绍各个功能模块细节。

### 2.6.1 引起数据变化的源头
在应用程序中，引起数据变化的应用场景大致有这几种情况，一类是用户的行为操作，即页面操作所引发的用户事件，如click, change, hover等，以此对用户的操作做出响应；另一类是前后端的数据交互，如从远端服务拉取页面所需要的接口数据，如`xmlHTTPRequest/webSocket`等，这是一个异步的过程，获取到一份较之前可能有变化的数据；最后一种是各类定时任务，即在某个延时后来响应对应的操作，对页面数据做出改变，如`setTimeout, setInterval, requestAnimationFrame`，他们都是在某个延时后触发。

以上三种可能导致数据变化的共同特征是它们都是异步的处理，即需要使用异步回调函数句柄来处理相关数据操作，结论就是，任意的一个异步操作，都有可能在数据层面上发生改变，这会导致应用程序状态被改变，因此，如果可以在每一个异步回调函数执行结束后，能够通知Angular内核进行更改检测，那么任何数据的更改就可以被实时的在视图层反应出来。
...
该例子是一个我们通讯录收藏的功能实现，当用户点击收藏按钮后，这将告诉`Angular`此时的联系人详情中的收藏数据`collect`发生改变，通知视图层做出相应的改变等。

### 2.6.2 变动通知机制
上一小节介绍了可能引起页面数据的几种场景，通过异步处理来通知页面重新更新数据源，因此，如果可以在每一个异步回调函数执行结束后，能够通知Angular内核进行变化监测，那么任何数据的更改就可以被实时地反应出来，但是，Angular本身是不具备这样的通知机制的，为了实现这种机制，引入了`NgZone`这个服务。

在Angular环境内注册的异步事件都运行在`NgZone`上(因为NgZone拥有整个运行环境的执行上下文)，它拓展了自己的一些API并添加了一些功能性的方法到它的执行上下文中，如`onTurnStart`和`onTurnDone`事件等，这些功能性的方法称作钩子（hook），当有异步操作发生、完成或者抛出异常时，会有对应的钩子对其进行捕获，以此来处理一些操作，比如说，在每次启动或者完成一个异步的操作、进行堆栈的跟踪处理、某段功能代码进入或者离开`NgZone`，可以在这些关键的节点重写所需处理的方法。为了更好的理解`NgZone`这个服务，本文先介绍Zone。

#### Zone相关介绍
Zone实际上是Dart的一种语言特性，其是对Javascript某些设计缺陷的一些补充，简单的可以概述成Zone是一个异步事件拦截器，也就是说Zone能够hook到异步任务的执行上下文，以此来处理一些操作以便在这些关键的节点重写所需处理的方法。

##### Fork一个子Zone
在使用Zone相关方法之前，需要fork一份Zone的参考实例，以此来获取一系列的钩子（hook）方法，这些钩子方法允许用户改变在当前Zone的执行环境中运行的代码行为。下面举个例子来对Zone有个大致的了解，具体代码示例如下：
...
该例子中，在Zone的`run`方法执行的开始和结束会有对应的钩子方法进行处理，在执行环境中按照代码的执行顺序，最新看到的是`beforeTask`中的打印，接下来是最后边的打印，待到`setTimeout`方法执行后，完成整个任务并执行`afterTask`方法，该JavaScript代码的执行结果如下：
...
通过对这个例子的理解，这里必须强调的是Zone的一个重要意义在于，所有的功能或者业务代码运行在一个fork的Zone中，该子Zone有了对该代码块执行上下文的控制权。其中也提供了一些钩子(hook)来处理基本的业务情景需求。

##### Zone的理解

Zone中提供了各类钩子，允许在每一个回调函数的开始和结束时，去执行统一的自定义逻辑，其本身是不做任何事的，相反它是依赖其它的代码，获取到这些代码片段的执行上下文，通过钩子来完成相关的功能。Zone的另一值得一提的是它必须依赖异步操作，当一个异步操作在执行时，它能捕获到这个异步操作并在该异步功能开始、完成或者有异常抛出时建立对应的回调函数，然后存储到当前的Zone，举个例子，在fork的Zone中执行一段代码片段，并且这段代码中包含一个`setTimeout`的异步任务，那么执行和完成这个方法需要包裹一个异步的回调函数，存储到当前Zone，这样是确保每个异步操作之间的相互不受影响，也就是受保护的状态。当fork了多个Zone之后，异步操作将会精准的执行其所在的子Zone上面方法。

Zone的一个重要意义在于，所有的功能或者业务代码运行在了fork的一个Zone中，该子Zone有了对该代码块执行上下文的控制权。其中也提供了一些钩子来处理基本的业务情景需求，常用的钩子大致有：

- `Zone.onZoneCreated`：在Zone被fork时运行
- `Zone.beforeTask`：在执行`zone.run`包裹的函数之前调用
- `Zone.afterTask`：在执行`zone.run`包裹的函数之后调用
- `Zone.onError`：`zone.run`方法中的任务抛出异常时的勾子函数

Zone是获取到了一个异步操作的执行上下文，并且证明了在错误处理和分析非常有用，通常情况下，这些的异步处理，比如说是服务端异步返回所需要的数据，或者是一些异步事件更改视图模型的数据等，通过`beforeTask`和`afterTask`等勾子函数能够很好的在异步事件发生或者结束的时候，允许在这样的异步任务节点执行一些分析代码。

在一些复杂的业务逻辑场景下，上面提及的这些可能远远不够用，很多时候应用场景要比这个例子要复杂的多，Zone采用猴子补丁(Monkey-patched)的方式将JavaScript中的异步任务都进行了包裹，同样的这使得这些异步任务都将运行在Zone的执行上下文中，每一个异步的任务在Zone都是一个任务，除了提供了一些供开发者使用的钩子外，默认情况下Zone重写了并提供了如下的方法:
...
通过对Zone的相关介绍，应该能理解Zone的应用场景了，即实现了异步任务的跟踪分析和错误记录以便更好的进行开发或者deBug等。Zone的相关拓展请参考本章的附录部分，需要深入理解该部分内容，请自行前往学习。

#### Angular中的NgZone
下面来介绍NgZone。实际上，NgZone是基于Zone来实现的，Angular的NgZone是从Zone中fork了一份参考，它是Zone派生出来的一个子Zone，在Angular环境内注册的异步事件都运行在这个子Zone上(因为NgZone拥有整个运行环境的执行上下文)，它拓展了自己的一些API并添加了一些功能性的方法到它的执行上下文中，这些勾子方法如`onTurnStart`和`onTurnDone`事件也会在该子Zone的`run`方法中触发。另外值得提出的是，在NgZone的TypeScript定义中，并没有把这个子Zone给暴露出来，因此只能通过ngZone的提供的方法来间接地对该子Zone进行操作。

NgZone提供了一些我们能订阅的自定义事件，具体如下：

- `onTurnStart`：在Angular事件启动前通知订阅器。
- `onTurnDone`：在Zone完成当前Angular任务时立即通知订阅者。
- `onEventDone`：在完成`onTurnDone()`回调之后在VM事件之前立即通知订阅者，常用来测试验证应用程序状态。

NgZone提供的这些自定义事件在跟踪定时器和其他微任务时是非常有用的，由于NgZone其实只是全局Zone的一个fork，Angular对于在Zone内需不需要执行变化检查，都具有完全的控制权，这为什么是有用的？因为我们并不总是希望Angular每一次都去执行变化监测，具体会在下面的小节详细讲解。

除此之外，Zone几乎在任何浏览器的全局异步操作打上了猴子补丁，并且NgZone是从Zone中fork出来的一份参考，它也拥有这些猴子补丁的钩子方法，例如当异步操作发生时就会通知框架进行变化检测，例如当类似于mousemove事件发生时，它也将引发变化检测。

在Angular源码中，有一个`ApplicationRef`类，其作用是用来监听NgZone中的onTurnDone事件，不论何时只要触发这个事件，那么将会执行一个`tick`方法用来告诉Angular去执行变化监测，具体的`ApplicationRef`类如下：
...
### 2.5.3 变化监测的响应处理
通过对前面小节的理解，我们已经知道触发Angular执行变化监测的时机，本节将深入理解Angular内部是如何对数据变动进行变化监测的。

#### 变化检测的处理机制
Angular应用由大大小小的组件组成，这些有相互依赖关系的组件组成了一颗线性的组件树，重要的是，每一个组件都有自己的变化检测器，由此组成了一颗变化监测树，这种变化监测树是线性流向的，数据的流向是从上到下，这是因为变化监测的执行总是由根组件开始，从上到下的监测每一个组件的变化，单向的数据流能很清楚视图中数据的来源，也就是说这些数据的变化是来自于哪个组件数据变化的结果。

为了更清晰的理解这些概念，下面将结合通讯录联系人相关的例子来说明，该组件树如下图所示：

![](./../_images/chapters2-2/component-tree.png)

图 2.6-2

在联系人列表中，由组件`ContactList`获取到联系人列表数据，在其子组件`ListChildrenComponent`展示具体的联系人数据，具体联系人详情信息在`ContactDetail`中展示，在联系人详情中有两个子组件，这两个组件分别是收藏联系人功能的`Collection`组件和编辑联系人的`EditContact`组件。如前面所讲，当一个异步事件发生并导致其中组件数据的改变，在组件中绑定的相关处理事件将会被触发，事件句柄处理完成相关逻辑之后，NgZone将会执行对应的钩子函数并通知Angular去执行一次变化检测。

每个组件中相互关联形成了如上图所示的组件树，组件树中的每个组件都有其对应的变化检测器，这确保了每个组件的变化监测相互独立，我们可以更灵活的控制变化监测的执行或者暂停等，这在性能提升上是有重要意义的。

那么在这个组件树中，变化检测器是如何处理的呢？在JavaScript语言中不提供给我们对象的变化通知，所以Angular必须保守的要对每一个组件的每一次运行结果执行变化检测，当组件中数据有变动的时候，`NgZone`通过钩子捕获到变化并通知Angular去执行变化监测，变化监测是单向线性的，即从根组件开始，依次触发各个子组件的变化监测器去完成变化的对比工作。对于每个组件的执行环境中Angular都会创建一个变化监测类，该变化监测类能准确的记录每个组件模型的一份参考，以此来维护作为下一轮变化监测的参考标准。

默认情况下，组件模型中的数据变化都会导致整个组件树的变化监测，但其实很多组件的输入属性是没有变化的，没必要对这样的组件来一次变化监测，如何减少不必要的监测，下文将继续探讨。

#### 变化监测类（ChangeDetectorRef）
Angular在整个运行期间都会为每一个组件创建变化监测类，其中提供了相关的方法来处理或者控制变化监测。正如前面讲述的那样，当有变化的时候，Angular会由根组件到子组件来监测每个组件是否有变化，因为Angular并不知道是什么地方出现了变化，但是我们是清楚具体的那个组件有变化的，所以可以标记并仅仅来监测这个路径上的组件即可，这就是变化监测类发挥它作用的时候了。
...
- markForCheck()：在组件元数据声明了`changeDetection: ChangeDetectionStrategy.OnPush`的组件中，变化检测是不会再执行的，必须加上该方法才会生效。
- detach()：从变化监测树中分离变化监测器，改组件的变化监测器将不再执行变化监测，除非再次手动执行reattach()方法。
- reattach()：把分离的变化监测器重新安装上，使得该组件及其子组件都能执行变化监测
- detectChanges()：从该组件到各个子组件执行一次变化监测。

其中的功能方法相互搭配才能在变化监测的时候体现它们的价值和意义，下面将通过例子来说明，假设通讯录中联系人数据来源时刻在变化，而产品需求又不需要实时的根据变化来展示数据，那么为了性能的考虑，可以设置在一定时间范围内来执行变化监测从而将改变体现在视图上，那么可以通过`detach`和`detectChanges`方法配合使用来实现性能的优化，具体代码示例如下：
...
@Component({
  ...
  template: `
  	<ul class="list">
	  <li *ngFor="let contact of contacts">
	    <list-li [contact]="contact"></list-li>
	  </li>
	</ul>
  `,
  ...
...
例子中通过依赖注入的方式注入`ChangeDetectorRef`变化监测类，`detach`方法用于从当前组件分离变化监测器，在需要变化监测的时间点再通过`detectChanges`重新发起变化监测，这样的处理方式在大数据列表中做性能的优化是有意义的。

#### 变化监测的策略
在Angular中，每个组件中都包含一些元数据，其中的一些是可选的，例如本节要介绍的`changeDetection`，它的功能是让开发者定义变化监测的功能。在使用该功能前，需要导入`ChangeDetectionStrategy`，一个完整的应用具体如下：
...
目前常用到的`ChangeDetectionStrategy`枚举类型有如下七类：
...
在开发当中比较常用到的是`Default`和`OnPush`，下面将具体介绍这两种枚举类型。

这里通过通讯录的例子来说明`Default`和`OnPush`，设想用户更新联系人的情况，如变化监测小节前面的图，变化监测会由根组件到子组件进行一轮变化监测，当`ChangeDetectionStrategy`的枚举类型是`Default`时，在触发了编辑按钮后，不管用户最终有没有修改数据，变化监测都会完整的进行一次，并且会对其中的具体参数与先前的参考进行对比。

当`ChangeDetectionStrategy`的策略为`OnPush`时，这将会通知Angular的组件仅仅依赖于它的输入属性的变化来做出变化监测，最常用的方式就是将输入属性的数据源当做一个不可变对象(Immutable)，当输入属性做出变更后，相对旧有的对象，我们将会保存另一份新的参照，以便下一轮变化时提供参考对比。当然基于性能方面的考虑，开发者们也会使用`Observables`机制来提升监测性能，具体我们将通过下面的一些应用场景来介绍变化监测的性能优化。

##### 使用`immutable`提升监测性能
不可变对象（immutable）给我们提供的保障是输入属性（@input）的对象结构不会改变，即当其内部的属性发生变化时，相对旧有的对象来做对比，同时将会保存另一份新的参照。它仅仅依赖输入的属性，也就是当输入属性没有变动，Angular将跳过对该组件的全部变化监测，直到有输入属性变化为止。如果需要在Angular中使用不可变对象，需要做的就是设置组件元数据`changeDetection`的`ChangeDetectionStrategy`的监测策略为`OnPush`，如下的例子：
...
例子为我们的通讯录联系人，`contact`对象仅仅依赖它的输入属性，同时也设定了`ChangeDetectionStrategy`变化监测策略为`OnPush`来告诉Angular如果输入属性没有任何变化的话，则跳过对该子组件的变化监测。

使用不可变对象明显的改善了庞大的组件树的性能问题，当设置了`ChangeDetectionStrategy`变化监测策略为`OnPush`后，在不可变对象的属性不被改变的情况下，变化监测将会跳过对该子组件的对比监测，只对比监测有改变的部分，相对于默认情况下对整个组件树的来一轮完整的变化监测来说，性能的提升将会有提升的。

##### 使用`Observables`提升检测性能
使用`Observables`机制提升检测性能和不可变对象类似，但它们有相关变化的时候不会提供一份新的参照，可观测对象在输入属性发生变化的时候来触发一个事件来更新组件视图，同样的，也需要给`ChangeDetectionStrategy`变化监测策略为`OnPush`，其目的是跳过子组件树的监测器，那它是如何通知是否需要进行变化检测的呢？先通过下面给这样的例子来说明变化监测：
...
该组件是通讯录例子中获取联系人并展示到视图具体功能实现方式，确切的讲，`ContactList`设置了一个属性绑定`contact`和一个可观测的对象`getContacts`，该组件在其`ngOnInit`生命周期勾子中订阅了一个事件流来获取当前的联系人列表，获取到整个联系人列表后，将会触发这个事件流来执行变化检测。

在这个例子中，设置了变化检测策略为`onPush`，那么在整个运行期间，都不会对子组件执行变化检测了，在输入属性有变化的时候，视图也不会再更新，除非注入`ChangeDetectorRef`服务并加上`markForCheck`方法，否则的话达不到这个例子想要的目的了。

变化监测总是由根组件到子组件这样一个从上到下的顺序执行的，如果要做变化监测优化其实是要准确的知道当前变化的子组件到根组件的具体路径并且变化监测仅仅就只按照这个路径来监测是否有变化，而不是整个组件树全部监测一遍。可以通过依赖注入给组件来引入一个`ChangeDetectorRef`，其中包含该一个`markForCheck`的标记方法，这个方法能标记有数据变化的子组件到跟组件的具体路径，那么上例中变化监测的路径大致如下图所示：

![](./../_images/chapters2-2/component-tree-2.png)

图 2.6-3

这样做的好处是，变化监测系统将不会走遍整棵树，你可以利用他们来监测树在局部是否有真正的改变，以此来做出相应的改变。

通过对以上变化监测的讲解，变化监测原理大致总结为：Angular应用是一个是响应系统，变化监测系统是由根组件到子组件的方向进行变化监测，它是是一颗线性的有向树，默认情况下，变化监测系统将会走遍整棵树，但是可以使用变化监测类的相关方法和使用不可变属性或者可观测对象搭配使用，可以实现真正的局部变化监测，这些优化组合方式同样的不会扰乱现有提供的变化监测保障。
...
&emsp;&emsp;在本章的内容中已对 selector、inputs、outputs、providers、viewProviders、 templateUrl、template、styleUrls、styles、directives、pipes、changeDetection 等元数据做了不同程度的讲解，以下将解析前面未提及的元数据。
...
#### 深入理解Zone（可选）
通过上一小节可以看到，每一个Zone的钩子统一服务于该Zone下面的所有回调函数。在实际开发中，可能需要对回调函数分门别类，对不同类别的回调函数采用不用的处理方式。于是Zone引入了子Zone的概念：一个Zone可以包含多个子Zone，每个子Zone都有其独立的钩子，从而不同类别的回调函数就可以添加到不同的子Zone中去。

##### 创建子Zone
每个子Zone都是通过调用zone.fork()方法来创建，是父类Zone的一个实例，每一个Zone都有一个唯标识符，同时还可以通过parent属性来寻找上一级Zone。`zone.run()`方法接收一个方法作为参数，在该方法内，可以给异步事件添加回调函数，从而这些回调函数会被该Zone所识别和统一管理，所有的业务代码都在`run`方法中执行，该`childZone`获取到了该业务代码的执行上下文并能通过钩子函数处理一些异步操作，还是用上面的例子说明如下：
...
##### 丰富Zone的钩子方法
很多情况下，我们需要对钩子方法重写并实现一些新功能，如下面的代码示例，当重新fork并重写`afterTask`钩子方法后，前面父级的钩子方法将会被覆盖。
...
// cya l8r
```
当重新fork一份新的Zone之后，通常会需要保留父级以及当前子Zone中相关的钩子方法独立的发挥其应有的功能，要实现这样的需求，只需要在子Zone的钩子方法前加上前缀`$`即可，而调整父Zone和子Zone中钩子方法的执行顺序等，可以通过语法糖`+`和`-`来实现，这里不再举例说明。
...
```
`childZone`没有定义`enqueueTask`勾子，因此在用`setTimeout`添加回调函数的时候，会沿着原型链向上寻找，这里其实是调用跟（root）zone的钩子。值得注意的是，虽然调用的是跟zone的钩子，但是this指向的仍然是`childZone`。但是如下面的例子中在`childZone`中定义上`enqueueTask`勾子函数，那么在原型链上最先找到的就是子Zone中定义的`enqueueTask`勾子函数。
