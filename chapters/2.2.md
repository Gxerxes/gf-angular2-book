...
&emsp;&emsp;在这种情况下，就可以考虑把它改造成自定义内容的复用组件。这里以一个简化的例子，来说明这种场景。假设有一个“绿色边框”的组件，叫 GreenBoarder，我们把这个组件的外观统一成绿色的边框，无论里面的内容怎么变化。假设它的模板是这样子：
...
&emsp;&emsp;然后，在其他地方使用了这个组件：

```html
<black-border>
  <h1>This is the content of green board!</h1>
</black-border>
```
...
&emsp;&emsp;在本章的内容中已对 xxx 元数据做了讲解，以下只对本章未提及的元数据解析。
