...
### 2.6.1 引起数据变化的源头
在应用程序中，引起数据变化的应用场景无非就那么几种情况，一类是用户的行为操作，即页面操作所引发的用户事件，如click, change, hover等，以此对用户的操作做出响应；另一类是前后端的数据交互，如从远端服务拉取页面所需要的接口数据，如`xmlHTTPRequest/webSocket`等，这是一个异步的过程，获取到一份较之前可能有变化的数据；最后一种是各类定时任务，即在某个延时后来响应对应的操作，对页面数据做出改变，如`setTimeout, setInterval, requestAnimationFrame`，他们都是在某个延时后触发。

以上三种可能导致数据变化的共同特征是它们都是异步的处理，即需要使用异步回调函数句柄来处理相关数据操作，这带给我们的结论就是，任意的一个异步操作，都有可能在数据层面上发生改变，这会导致应用程序状态被改变，因此，如果可以在每一个异步回调函数执行结束后，能够通知Angular内核进行更改检测，那么任何数据的更改就可以被实时的在视图层反应出来。
...
通过对Zone的相关介绍，应该能理解Zone的应用场景了，即实现了异步任务的跟踪分析和错误记录以便更好的进行开发或者deBug等。
