...
provider在前后台各种技术领域都被广泛使用，从.Net的MembershipProvider，Hibernate的ConnectionProvider，到Android的ContentProvider，本书讨论的Angualr2 Provider，虽然实现过程不尽相同，但核心概念都类似，Provider作为封装了逻辑操作或数据操作的提供者，提供了接口或数据供调用方使用，相比接口继承实现模式,Provider模式提供了更多的可扩展性和灵活性。
在Angular2中，注入器的依赖项总是一个Provider对象，考虑到平常使用的场景大多是类作为提供者，框架会自动把类名转为Provider对象，上面在在contract-app.ts中注册的Logger服务，只需要简单的往组件元数据的providers数组中添加Logger服务

providers: [Logger]
这是框架实现的语法糖，等同于

providers: [new Provider(Logger,{useClass:Logger})] 
Angular2自动根据类名生成了Provider对象，实际上还有其它生成Provider的方法，用provider函数构造
...
下面将以一个电话薄（AddressBook）的例子，来讲解依赖注入的使用。
    ...
    export class AddressBook {
		...
		addressBookTitleService:AddressBookTitleService){
...
从上面的例子可以看出，给组件AddressBook中注入了两个服务，addressBookService和addressBookTitleService。这两个服务是两个简单的类，如下所示:
...
这一步，还需要通知Angular2.0把依赖addressBookService和addressBookTitleService注入到组件AddressBook中。在Angular2中，在providers中说明：在装载程序的过程中，实现方式有两种：组件级别（ component level）和app级别（app level）。

为了做对比，AddressBookService采用组件级别的注入
 ...
 AddressBookTitleService使用app级别的注入

    bootstrap(MyDemoApp,[AddressBookTitleService]);

这两种注册依赖的方式都可以实现通知angular当前类可以作为依赖被注入。但是它俩还是有些区别：以app级别的方式注册的依赖AddressBookTitleService在整个应用的生命周期是以单例的方式存在，而以组件级别的方式注册的依赖AddressBookService将针对每个组件生成一个新的实例。由于在组件级别的注入中，每个有注入依赖的组件都会生成一个新的对象，所以就不能在app范围内使用这些依赖来存储状态。
