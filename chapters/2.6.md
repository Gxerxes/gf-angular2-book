...
3. 当需要测试Robot类时，还需要考虑Robot类隐藏的依赖。例如：Head组件本身是否依赖于其它组件A，且它依赖的这个组件A是否也依赖于另一个组件B。同时Head组件的实例是否发送了异步请求到服务器。因为不能控制Robot的隐藏依赖，所以使得Robot很难被测试。
...
清楚了Angular2依赖注入中的Injector、Provider和Dependence。下面就来分析如何通过依赖注入来获得一个Robot的实例， 以便更好的理解三者之间的关系。
...
上面两步实现了最基本的在组件中使用依赖注入，对组件开发者来说无需关心如何初始化，如何创建，框架在运行时帮你注入，很是方便。

下面通过通讯录（AddressBook）的例子，来讲解组件中注入依赖的使用。
...
实际上，第一步在component注解中providers属性配置依赖项并不是必须的，前提是你已经在跟组件中注入过，这是Angular依赖注入另一个强大的功能`组件树`，本章后面会详细介绍。

下面的例子也是组件中注入依赖的一个例子，我们并没有在component注解中添加providers属性，它也能在构造函数中获取到contractService的实例，因为已经在根组件ContactApp中注入了依赖ContactService。
...
类似上面的组件中注入service，service中注入service也需要三步：

1. 通过import导入用于创建被依赖对象的类
2. 在需要注入依赖的service上配置注解@Injectable()
3. 在需要注入依赖的service的构造函数中注入被依赖的对象
...
结合组件中注入service的例子，可以看到Angular首先创建了ContactApp，然后依赖注入框架生成LoggerService的实例，在创建ContactService时，它依赖服务LoggerService和UserService，LoggerService的实例已经生成，UserService还没有，紧接着依赖注入框架生成UserService的一个实例。这涉及到了嵌套服务注入，虽然有点复杂，幸运的是我们不需要处理这些关系，只需在构造函数中声明需要的依赖，其余的工作交给依赖注入框架就可以了。
