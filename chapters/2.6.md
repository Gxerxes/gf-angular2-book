...
在Angular2中，一个应用会涉及到很多组件和服务，这些组件和服务之间会有千丝万缕的联系，如何很好的管理它们之间的依赖关系，往往是一个比较棘手的问题，这也是体现一个框架是否强大的“硬指标”，Angular提供的依赖注入（DependencyInjection）机制，可以优雅的解决了以上问题。在传统的软件开发中，由调用者负责管理所依赖的被调用者，而在依赖注入模式中，这个管理权交给了注入器，注入器能在软件运行时负责被调用者的替换，而不是在编译时，这种控制反转，运行注入的特点即是依赖注入的精华所在。  
本章从最简单的依赖注入开始，首先介绍了什么是依赖注入，接下来重点剖析了Angular2的依赖注入，然后通过例子说明Angular2中如何使用依赖注入，主要包括四部分：组件中注入Service、Service中注入Service、层级注入的使用和可选注入，最后分析了Angular2依赖注入的Providers的五种注入形式。通过本章的学习，读者对依赖注入会有一个系统的认识。
...
有了注入器(Injector)，Robot就不需要知道如何创建它的依赖Head和Arms，用户也不需要知道怎么去生产一个Robot，同时也不需要维护一个巨大的工厂类，下一小节我们将开始讨论Angular2中的依赖注入，看它是如何实现的。
...
注入器Injector是如何知道初始化一个Robot组件需要创建那些依赖，答案在这里：DI（依赖注入）从Robot的构造函数可以知道创建一个Robot所需的依赖dependence；providers则说明了如何创建这些依赖。把resolveAndCreate()方法进行扩展就可以看到provide的影子。
...
我们注意到在类LoggerService和类ContactService中，都用了@Injectable()，它到底有什么用呢？@Injectable()作为装饰器存在，只有对有依赖的服务类才是必须的。上例中LoggerService没有依赖其它的服务，所以这里@Injectable()不是必须的。ContactService服务依赖了其它的服务，所以@Injectable()是必须的。

Angular2推荐任何情况下都使用这个注解，一方面任何service都有可能依赖其它组件，即使现在看起来并不需要。另一方面，这也是一种好的团队协作约定，使用的时候无需去关注service是否可注入这个问题。

## 6.3.3 层级注入

Angular2的开发模式以组件为基础，开发中自然而然形成层层组件的嵌套，这种组织关系组成了一颗组件树，一个根组件下面是各层级的子组件，依赖注入就每颗树上结的果实，可以出现在任何层级的任何组件中，每个组件可以拥有一个或多个的注入（结多个果实），每个注入对于每个**注入器（injector）**而言都是**单例（singleton）**（每个果实都不一样）。
...
provider由来已久，在前后台各种技术领域都被广泛使用，从.Net的MembershipProvider，到Hibernate的ConnectionProvider，再到Android的ContentProvide。本书讨论的Angualr2 Provider，虽然其实现过程不尽相同，但核心概念都类似，Provider作为封装了逻辑操作或数据操作的提供者，提供了接口或数据供调用方使用，相比接口继承实现模式,Provider模式提供了更多的可扩展性和灵活性。

注入器Injector把服务注入到组件或者其它服务中，用于注册服务的Provider来生成所需的依赖。Provider提供了一个具体的可运行的依赖，注入器Injector依靠Provider来创建依赖服务的实例。

为了便于讲解，这里依然使用通讯录的例子，通讯录应用里面有个记录日志的服务，日志服务LogService注册到providers中，通过LogService来生成一个Logger服务，如下所示：
