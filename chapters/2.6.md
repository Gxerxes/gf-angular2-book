#6 依赖注入 
##6.1 概述
上一章节介绍了服务的相关知识，我们知道一个应用会涉及到很多组件和服务，这些组件和服务之间会有千丝万缕的联系，如何很好的管理它们之间的依赖关系，往往是一个比较棘手的问题，一般需要考虑下面两个方面：

1. 如何设计一套松耦合且易于扩展和修改的组件？
2. 如何使代码便于测试？

Angularjs提供了依赖注入（Dependency Injection）机制，优雅的解决了以上问题。在传统的软件开发中，由调用者管理自己使用的被调用者，但是在依赖注入模式中，这个管理权交给了注入器，注入器能在软件运行的时候负责被调用者的替换，而不是在编译时，这也是依赖注入的关键优势。依赖注入模式除了应用于复杂软件的测试，还经常用于定位组件，或者定位和初始化软件中的服务。（扩充）
...
##6.2 Angular2中的依赖注入
###6.2.1 什么是依赖注入
依赖注入是angular的主要特性和亮点，它可以理解为一种设计模式。简单地说，Injector(注入器)将组件依赖的其他对象注入到这个组件中, 而应用组件并不用关心被依赖的对象从哪里获取，以及如何被创建或初始化。相比依赖的硬编码实现，依赖注入有利于各种组件之间解耦，从而获得易于维护的代码。这种优势可能一开始体现不出来，但当组件依赖很多其它组件或对象时，依赖注入就会帮你省了很多麻烦，提升了开发效率。
...
有了注入器(Injector)，Robot就不需要知道如何创建它的依赖Head和Arms，用户也不需要知道怎么去生产一个Robot，同时也不需要维护一个巨大的工厂类。

###6.2.2 NG2 依赖注入

讲到Angular2，首先介绍三个关于Angular2的概念。
