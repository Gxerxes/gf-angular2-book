...
由于ContactService已经在根组件中注入，所以组件collection中直接在构造函数声明即可。 如果没有注册到根组件，则需要转载@Component中通过providers注入到collection中，如下所示
...
在通讯录的例子中，目前的代码如下，假设需要对返回的通讯录列表排序，可以创建一个实现排序功能的子组件
...
``` 
上面针对之前的通讯录版本做了点修改，获取通讯录列表信息的代码移到了ngOnInit里，同时增加了afterGetContracts这个方法，在获取数据后调用排序，子组件重写了排序的具体实现功能，最终子类的collections就是排序后的了。
...
通讯录列表的例子中引入Logger服务，代码如下，因为Logger在顶层组件里已经注入，根据查找规则，最终会在顶层组件里查找到依赖，代码可以正常运行
...
在组件的章节中已经介绍了组件间通讯的几种方式，另一个问题是如何找到子组件或父组件的引用，在Angular2中，已经提供了API来获取子级组件的引用(Query, QueryList, ViewChildren, 和 ContentChildren) ，获取父组件的引用就麻烦些，没用现成的API，但因为每个组件的实例都会添加到注入器的容器里，因此可通过依赖注入来找到父组件。
