...
上一节，通过Robot的例子讲解了依赖注入的概念，接下我们来分析Angular2的依赖注入。为了更好的理解Angualar2的依赖注入，首先介绍三个重要概念。
...
注入器Injector是如何知道初始化一个Robot组件需要那些依赖呢？这是因为依赖注入(DI)从Robot的构造函数可以知道创建一个Robot所需的依赖(dependence)；providers则说明了如何创建这些依赖。把resolveAndCreate()方法进行扩展就可以看到provide的影子。
...
一个token可以映射到任何想要的东西，这里把token:Head映射到类OtherHead，这样做有两个好处：
...
###6.3.2 组件中注入服务
Angular在底层做了大量初始化功能，磨平了创建依赖注入的大部分工作，对于使用者来说很是方便，使用依赖注入只需要三步：
...
依赖注入的过程总结为一句话：当组件初始化的时候，注入器根据构造函数上的声明，再结合第二步providers配置的依赖映射，来生成需要的依赖服务。至此依赖注入完成，可以在该组件中使用被注入的服务了。

下面通过通讯录的两个例子，来讲解如何”三步法“实现在组件中注入服务。
...
一般情况下，每个组件都有自己的注入器，注入到该组件服务都维持单例，这跟Angular1.x不一样。如上例，组件`ContactApp`中注入了依赖：logger、contactService和userService，这些service都是单例。`@Component`注解中通过`providers`配置依赖，该配置不是必须的，如果已经在根组件配置过相关的依赖，则该组件中就不需要再做配置了，这就涉及到了Angular2依赖注入另一个特性`层级注入`，本章后面将详细介绍。下面的例子我们并没有在`@Component`中添加`providers`属性，但是依然可以在构造函数中获取到`ContactService`的实例：
...
由于`ContactService`是一个系统级别的通用服务，更适合在根组件`ContactApp`注入，这样子组件们无需再创建相应的实例，更多讨论请看后面的组件树。

### 6.3.3 服务注入服务

除了组件中注入服务，服务间的调用也是常见。服务各司其职，提供对外接口供第三方调用。在通讯录的例子中，当底层服务异常的时候想记录下错误信息，这就需要一个log服务来帮忙。下面是log服务的的简单版，实现把log信息打印到console平台。
...
类似于上面的组件中注入依赖，服务中注入服务，也需要三步：
1. 通过import导入被依赖对象的服务。
2. 在需要依赖的服务上添加注解`@Injectable()`。
3. 在服务的构造函数中声明被依赖的对象。

下面通过`ContactService`服务中注入`LoggerService`服务来说明：
...
```
在上面的例子中，我们注意到`LoggerService`类和`ContactService`类都使用了注`@Injectable()`，它到底有什么用呢？`@Injectable()`作为装饰器存在，它只对需要依赖注入的服务类才是必须的。上例中`LoggerService`没有依赖其它服务，这里的`@Injectable()`不是必须的,而`ContactService`服务依赖了其它服务，这里的`@Injectable()`是必须的。

> Angular2官方推荐任何情况下都使用注解`@Injectable()`，一方面任何服务都有可能依赖其它服务，即使服务当前没被注入依赖，但是后面有可能需要注入依赖，如果任何服务都提前加上注解`@Injectable()`，就不需要时不时考虑某个服务是否可以注入依赖了；另一方面，这也是一种好的团队协作约定，整个团队遵循相同的开发原则。
...
> 实际上，每个组件都有独立的injector，由这个injector创建的依赖对象都是单例，这是组件级别的单例，跟Angular1.x不一样，有1.x开发经验的读者会更有体会。
...
上面例子的运行结果说明了子组件共用了父组件的实例，这就不能满足每个通讯录项有唯一标示的需求。Angular2这种灵活设计引申了另一个问题：依赖什么情况下注入到根组件，什么情况下又该在组件中注入?
...
> 这种树形的从子孙往上查找直到根节点的设计在各个语言里很常见，面向对象里获取某个子类的属性，也是向上层层查找直到根对象，在JavaScript里的原型链查找过程同样如此。


### 6.3.5 注入到派生类

对于有继承关系的组件，当父类组件和`派生(inherit)`类组件有相同的依赖注入时，父组件有注入依赖的同时，派生组件也需要注入这些依赖，并在构造函数中往上传递。
...
上一节通过例子说明了不同形式的依赖注入，Provider这个概念一直有意不去讨论，这一节来单独讲解Provider。
...
1. 组件中注服务：组件中注入服务的“三步法”：第一、通过`import`导入被依赖对象的服务；第二、配置注入器`providers`；第三、组件中声明注入依赖。
1. 服务中注入服务：服务中注入服务也是三步完成：第一、通过import导入被依赖对象的服务；第二、在需要依赖的服务上添加注解`@Injectable()`；第三、在服务的构造函数中声明被依赖的对象。
