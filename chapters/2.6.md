...
注意@Injectable()这里，这个注解不是必须的，因为当前LogService没有注入其它服务，作为好的开发习惯，最好还是保留，后面会讨论到是否保留这个话题。接着，这个Log Service就可以给ContractServices注入了。
...
注意这里的**@Injectable()**就是必须的了，用**provider**注册服务到组件中使得Angular2能够创建具体的Logger实例，并注入到ContractServices中。考虑Logger这种通用的服务应该是系统级别通用的，在根组件注入是更很合适，这样整个系统都可以共有。在contract-app.ts中，增加Logger到provider数组中
...
前面提到了@Injectable()，那到底什么时候class才需要@Injectable()？Angular2推荐任何情况下都使用这个注解，一方面任何service都有可能需要inject，即使现在看起来并不需要。另一方面，这也是一种好的团队协作约定，使用的时候无需去关注service是否可注入这个问题。
