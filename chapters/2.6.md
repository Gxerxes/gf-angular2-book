...
```
首先定义一个别名provider，拥有useExisting定义的provide函数，它新建一个备选的方式来注入同一个组件实例，并把这个供应商添加到ParentComponent的@Component元数据里的provider数组。

Parent是该供应商的类-接口令牌，因为ParentComponent 引用了自身，会造成循环引用，用forwardRef可避免该循环，forwardRef的使用后面会讨论到，目前，上面例子中ParentComponet的注解变成这样
...
这样，在子组件中就可以通过Parent这个provider找到父组件的实例了。
