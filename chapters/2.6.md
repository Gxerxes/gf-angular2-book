...
在本章中我们介绍了依赖注入的基本容器概念，即将依赖的对象注入到组件中, 而这个应用组件并不用关心被依赖的对象从哪里获取，以及如何被创建或初始化。重点剖析了Angular2的依赖注入，介绍了Angular2依赖注入的三个重要概念：Injector(注入器)、Provider(容器) 和Dependence(依赖) 。

然后通过例子说明Angular2中如何使用依赖注入，主要分为五部分：

1. 组件中注入Service：组件中注入service即在一个组件中注入依赖的另一个组件，主要包括两步,第一：通过Provider配置注入器，第二：组件中声明注入依赖。 
1. service中注入service：一个service可能需要另一个service提供的服务，这就用到了service中注入service。它的注入方法与组件中注入service类似。 
1. 层级注入：Angular2以组件为基础，开发过程中会涉及到组件的嵌套和继承。此时子类组件和父类组件可能都需要来某个服务，这是只需要父类组件注入依赖，子类可以直接使用这个依赖，不需要再注入一遍。 
1. 注入到派生类：在继承关系的组件中，当父类和子类有相同的依赖注入时，父组件注入依赖的同时，子组件也需要注入这些依赖。
