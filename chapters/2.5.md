...
介绍完了面向变化传播，再来看看什么又是面向数据流的编程。顾名思义，当我们监听一系列事件流（例如点击事件流）并对这一系列事件流进行处理（例如，合并，过滤等操作）最后再响应整个事件流的回调，这样一个过程便属于面向数据流的编程。

例如，在下文将会介绍到的ReativeX的编程范式中，整个数据流被封装抽象在一个叫做Observable的对象实例中

```
observable.filter(Iterator).subscribe(callback);
```

通过观察者模式，对数据流进行统一的订阅（Subscribe），并在中间插入像filter这样的操作函数，对observable所封装的数据流进行过滤处理。这样便实现了面对数据流的响应式编程。

通过这些例子，可以看到，通过一种简明扼要的同步的方式，方便地表达了静态或动态的异步的数据流，而相关的计算模型也自动地将变化的值通过数据流的方式进行了传播。这样的编程方式便是响应式编程。
...
####Observable
在Rx中，最核心的概念就是Observable。Rx继承了观察者模式的理念，所有的异步数据流都被封装在一个叫做Observable的数据结构中，并且在对这些数据流进行操作，重组，再构建的时候，允许我们把这些异步的数据流，看成一个同步的，已经构建好的Collection，使得这些异步数据流可以通过一些Collection的方法对整个异步数据流进行操作。

说的有点晦涩，用官网的珠宝图来做个解释：

![珠宝图](http://reactivex.io/assets/operators/legend.png)

最上面的珠宝轴（时间线）代表了最原始的数据流。这个数据流可能是代表一个按钮的点击事件流，每个珠宝便代表了一次点击事件；或者也可以是后端某个API接收到的某个请求事件流，每个珠宝便代表了一次请求。无论如何，在Rx里面，这样的事件流都会被看成是一个observable。这样的observable会随着时间不断抛出某个事件，虽然这些事件是随着时间的变化，异步抛出的，但在Rx中，把这些异步抛出的事件可以看成一个事先已经确定好的序列，也就是说，数据流被封装成的observable更像是一个collection。

在上图中，可以看到，原本的observable，或者说事件流，在经过了flip操作之后，生成了一个新的observable，或者说生成了一个新的事件流。这样的过程是不是很像对一个collection做了一个过滤操作，生成了一个新的collection？

Rx便是借鉴了Collection的某些操作思想，把复杂的异步数据流的处理问题，简化成了同步的Collection的处理问题。换句话说，在Rx中，原本表示异步数据流的Observable最终被当成了一个同步的已经存在的Collection而看待。
...
在Rx中，Observable作为观察者模式中的被观察者，或是被监听者，自然需要一个方法来被监听，或者说被订阅，而subscribe便提供了这样一种方法，监听observable发出的所有事件。
...
在被subscribe订阅过的observable并不会返回一个新的observable，毕竟这不是一个可以改变原始数据流的函数，取而代之，subscribe会返回一个Subscription的实例，这个Subscription的实例又会对应很多方法对其进行操作，例如unsubscribe，这里就不做过多的展开了。
...
由于篇幅有限，在这里就不对这些概念进行过多的展开，有兴趣的话，可以去Rx的官网进行更多的了解和学习。

虽然介绍了这么多Rx的概念，但由于Rx仅仅是个编程范式，这种编程范式可以在多种语言平台上实现，所以不好列出具体的某种语言的代码对这些概念做语言层面的讲解。而被大众所熟知的RxJS，RxJava便分别是Rx在JavaScript和Java中的对应实现。在下一章节介绍RxJS时，我们再来看看在具体的代码层面，如何把数据流封装为统一的Observable，再看看如何对这些Observable进行更实际化得处理。

##5.4 RxJS
RxJS，一个在JS语言中提供响应式编程的库，是Rx在JavaScript语言层面的实现。

###5.4.1 第一个例子
在上一章介绍了一大堆的Rx相关的概念后，我们这一章先来看看如何用RxJS来构造一个简单的Observable实例。
...
在实际使用中，RxJS在数据流的处理方面扮演的角色更像是一个可以对数据流进行处理的Loadash，而除了类Loadash的操作以外，由于RxJS强大的异步数据流处理的能力，使得他同时还能扮演Promise的角色，可以同时当做是Promise的超集的存在。

###5.4.3 RxJS VS Promise
说到对异步数据的处理，JSer们往往会先想到Promise。那RxJS和Promise做比较，究竟谁优谁劣呢？这里先给出结论：能用Promise的场景RxJS都适用，RxJS是作为Promise的超集的存在。
...
```
同理对于race等一系列其他的操作，RxJS都有对应的解决方案，这里也不做过多的赘诉。所以，RxJS是Promise+这种说法一点都不为过。

###5.4.4 “冷” 的Observable
在Rx的概念中，Observable可以有两种模式 “冷”（Cold）模式和 “热”（Hot）模式。在 “热” 模式下，observable一旦创建，便会开始发送数据。而在 “冷” 模式下，observable会一直等到自己被订阅，才会开始进入数据流的发送。而具体改Observable是哪种模式，取决于语言层面对其的实现。

在RxJS中，Observable是“冷”模式。看段代码：
...
// observable start
```
可以看到，在RxJS中，observer.next在obs被订阅后才执行。也就是说冷模式下的observable直到改observable被subscribe之后，才会进入数据发送的流程中。

不过除此之外，在RxJS中，还存在一种被称作Connectable的Observable，这种模式下的observable不管有没有被订阅，observable都不会开始发射数据的流程，除非connect方法被调用。
...
RxJS作为Rx的实现，自然把Operators作为自己最强的竞争力。这小节便节选出一些Operators做些细致的讲解。
...
下面来列举两个比较常见的场景，来展现在Angular2，RxJS的灵活使用。
...
可以看到，在上面的代码中，最核心的是在constructor里面，```term.valueChanges```返回了一个observable，这个observable会在每次input框里的value进行改变时，抛出value值，并被subscribe所订阅。监听到这个value会直接调用demoService中的getRecommend接口，返回需要的recommend数据。
