...
1. 属性指令
	...
	以往，为了控制DOM元素的样式，可以在HTML元素的`style`属性中，为元素指定CSS样式类。现在，有了属性指令，可以使用Angular内置的属性指令`NgClass`，来为元素同时的增加或删除多个CSS类。
	
	使用`NgClass`指令，需要将其绑定到一个key：value的对象。其中，每一个key表示一个CSS类名，value值是true或false。当value值为true时，元素增加这个CSS样式类，value值为false时元素移除这个样式类。
	
	假设在目标组件方法中有一个控制CSS类的状态的方法`setClasses()`如下
	...
	那么可以为`NgClass`属性绑定`setClass`方法，以此来设置元素的CSS类
	
	```
	<div [ngClass]="setClasses()">This div is saveable and special</div>
	```
	
	这样就实现了使用`NgClass`属性指令来设置DOM元素多个CSS类的效果。
...
3. 组件
	...
	在第二章中详细介绍了组件相关的内容，组件被用来构造带有自定义行为的可重用的视图。作为指令的一个重要子类，组件本质上可以看作是一个带有模板的指令。
...
| ngOnDestroy |在Angular销毁指令/组件之前执行清理工作。注销之前的观察并且解绑之前绑定的事件处理器以避免内存泄漏|
	...
	* 使用@Directive修饰符注册指令;@Component修饰符注册组件
...
* 平台指令 PLATFORM_DIRECITVES
	...
	平台指令是与其他指令集和不太一样，它不包含具体的指令。使用时，可以为平台指令定义一些具体的指令，这样，被添加平台指令集合的指令，在应用的任何地方便可以直接使用，而无需再在组件中声明引用。
...
在如下示例中，当需要使用的指令很多时，如果不使用通用指令集合，需要显示的依次引入所有需要的指令，并在组件装饰器函数的`directives`属性中，依次的声明所有指令。
 ...
 上述的使用非常的不便，为了改进这一使用过程，可以通过使用指令集合，一次引入所有的通用指令
...
本节对核心指令集合中`NgTemplateOutlet`和`NgPlural`指令进行讲解
...
* 表单元素访问器
	...
	这些访问器指令是Angular表单的内部指令，无需显示的使用它，它们是DOM元素和表单输入`control`之间的桥梁。其中`ControlValueAccessor`是其他访问器指令的父类接口，抽象了`control`与DOM元素交互的公共方法。
...
如前文所述，平台指令集合与其他指令集不同，它本身不包含具体的指令，而是可以看做一个象征，在应用启动时，可以将一些具体指令设置到平台指令中，然后就可以在应用的任何组件中直接使用它们。

在以往的使用中，为了使用某一指令，需要在组件的`directives`中定义引用，平台指令集合可以简化这一步骤。不过这点便利不是平台指令主要应用场景，当有一些指令可能在整个应用中频繁使用时，平台指令便可以提供更方便的引用。
...
上述代码示例的开始从Angular库中引入了`Directive`和`ElementRef`。 `Directive` 符号被用来标记`@Directive` 装饰器，`ElementRef`符号被用来将元素注入到指令的构造函数中，以便可以访问相应的DOM元素.

随后，在`@Directive` 装饰器函数中以对象形式定义了指令的元数据。在配置对象中，使用一个CSS选择器来标识该属性指令所关联的HTML模板，`[myBeautifulBackground]`是指令所对应的选择器 。当指令运行时，Angular会在模板中匹配所有包含属性名称`myBeautifulBackground`的DOM元素。
...
目前，在上述自定义指令中，为元素设置的背景色是硬编码的。为了增加灵活性，应当在指令外部，像下面示例，使用绑定的方式设置背景色。
...
## 4.3 结构指令
上一节中实现了一个自定义属性指令，使用它可以在点击元素时改变其样式。除了可以自定义属性指令，Angular还允许自定义结构指令，本节内容将进一步讲述结构指令的关键内容，并且实现一个自定义结构指令，作为示例，本节将实现一个和`ngIf`作用相反的指令。
...
`Unless`指令需要访问组件模板中元素，并且需要可以渲染它的内容的工具。`TemplateRef`可以用来访问模板，`ViewContainerRef`作为渲染器使用，在指令的构造函数中，将它们注入赋值给指令的私有变量。
...
### 4.3.2 模板标签与星号前缀
上一小节实现了一个自定义结构指令：Unless指令，当表达式值为false时渲染模板内容，值为true时将模板内容从DOM树中移除。本节内容会就结构指令的两个关键内容进一步讲解。
...
1. 模板标签
	...
	将上述片段分别在Angular环境和普通的HTML页面中使用，两者在浏览器的显示是一样的，通过使用Chrome DevTool工具查看生成的DOM树，可以看到在HTML页面中，`template`标签内容被`document-fragment`包装，而在Angular中，`template`标签和内容被移除了。
...
2. 星号前缀
	...
	它们的基本模式是一样的，创建一个`<template>`标签，然后将内容放到`<template>`标签内部，并将指令移到`<template>`标签中。标签中，`ngFor`是指令的选择器，`[ngForOf]`是指令的输入属性，这里将`heroes`集合绑定给了`[ngForOf]`作为指令的输入，`let-hero`创建了一个模板局部的输入变量`hero`。


### 4.3.3 ngIf指令原理

在本节中实现的`Unless`指令是一个和`ngIf`作用相反的指令，使用`Unless`或是`ngIf`时，根据绑定表达式的布尔值，在DOM树插入或者删除模板内容。

通过将元素CSS样式`display`设为为`none`来隐藏内容，这些元素虽然不可见却仍然存在DOM中。如果隐藏一个元素，组件的行为将会继续。它仍然保留在DOM中，继续监听事件。Angular会继续检测那些可能发生变化的数据绑定，组件的所有行为将会保持。在这种情况下，组件及其所有的子节点仍然会占用资源，消耗更多的性能和内存。
...
在4.3.1小节中，实现了`Unless`指令代码逻辑，在自定义`set`方法中，根据绑定的输入属性的值的不同，选择调用渲染器的`createEmbeddedView()`还是`clear()`方法，实现插入或删除模板内容的效果。

`Unless`指令代码的实现很简单，这里来看下`ngIf`指令的实现
...
上述代码中，可以看到`ngIf`指令的源码和`Unless`指令代码很相似。不同的地方在于，`ngIf`源码中，设置了`_prevCondition`变量记录上次表达式值，在每次输入变更时，检查`_prevCondition`变量，避免进行不必要的操作用以提高性能。

## 本章总结
本章讲述了Angular中指令相关内容，从Web开发者最熟悉的HTML元素与属性开始，认识到Angular指令并不神秘，它是对HTML元素及其属性的扩展。

在Angular中，共有三种指令。属性指令可以扩展元素的样式和行为，结构指令可以改变DOM元素的结构，而组件本质上是一种带有模板视图的指令。
...
在自定义结构指令时，使用`TemplateRef`引用模板元素，使用`ViewContainerRef`作为视图的渲染器，调用渲染器的`createEmbeddedView()`和`clear()`方法，添加和删除模板元素的内容。为了更进一步的理解，讲解了使用结构指令时，星号前缀背后的故事和`<template>`标签的原理。最后通过对`NgIf`指令源码的阅读，理解了其实现的原理。
