...
...
```
当页面视图中的姓名控件输入值其他值时，p标签中curContact.name值并不会改变。那如果反过来，直接在组件中修改初始化构建的curContact.name值，页面视图的姓名控件输入值是否会跟着改变？
...
```
当点击切换按钮时，在组件改变数据模型的curContact.name值为"张三"后，视图中姓名控件显示的值跟着变成"张三"。
...
```
例子中是一个选择性别的单选框，男和女单选框的`ngModel`绑定的是同一个值`curContact.sex`，当点击“女”单选框，`curContact.sex`会被赋值为“女”单选框控件所对应值“female”，同理，当点击“男”单选框，`curContact.sex`会被赋值为“男”单选框控件所对应的值“male”。若性别单选框需要设置默认值，如设置默认选中“男”单选框，可以在组件中给curContact.sex设置初始值为"男"单选框控件所对应值，即`curContact.sex = 'male'`。
...
####表单中的局部变量
模板中可以定义局部变量，模板表单中也可以定义局部变量，模板表单中的局部变量与模板中普通的局部变量有些区别。模板中普通的局部变量通过#定义，默认赋值为当前元素Dom对象的引用，能直接在模板中获取元素的属性值，如className、value等。模板表单中的局部变量在定义时会被赋值为表单指令对象的引用，能追踪表单的状态及有效性。
...
```
对于简单的AJAX请求提交表单采集的数据，这无疑是令人兴奋的功能，直接将contactForm.value提交即可，不需要自己构建JSON对象，非常简洁方便。
...
```
控件局部变量#name初始化为`NgModel`，而在定义值表单局部变量#contactForm时初始化为`ngForm`。 两者的区别是表单局部变量#contactForm为`NgForm`指令，是`NgForm`实例对象的引用，控件局部变量#name为`ngModel`指令，是`ngModel`实例对象的引用。
...
###3.6.1 管道的用法
使用管道操作符`|`，将获取到的原始数据作为输入数据，将转换后的期望数据作为输出数据并显示在模板中。下面将生日数据转换成用户友好的展示方式：
...
以月份为例，根据表格3-6-1自定义日期格式，若月份需要英文缩写的话，使用`MMM`。月份展示英文全称，使用`MMMM`。需要显示数字可以使用单标志符`M`。若月份是1-9月，希望显示成两位01-09，则使用双标志符`MM`。
...
我们在使用原生html5 date控件时，设置最大最小日期范围，输入格式必需为`YYYY-MM-dd`，月份及日期必需为两位，不足两位，添加前导0。在这种场景下，使用日期管道参数`y-MM-dd`，很方便得到对应的格式。
...
`UpperCasePipe` 将文本所有小写字母转成大写字母，用法比较简单，不再赘述。

**LowerCasePipe**

`LowerCasePipe ` 将文本所有大写字母转成小写字母，用法比较简单，不再赘述。
...
####全局自定义管道
上面定义的`DateReform `日期管道，不仅会在当前页面使用，也会在其他页面使用。对于这类通用的自定义管道，每次使用前都需要在当前页面import管道，然后在`@Component`装饰器的pipes数组里列出使用的管道，比较繁琐。为了避免这种使用场景，Angular提供了全局自定义管道，供全局使用。
...
### 3.6.3 管道状态
Angular在每次点击、移动鼠标、定时器触发、服务器响应等事件后会执行数据绑定值变化检测，这会带来性能问题，因此如果我们使用管道，Angular会选择使用更简单、更快速的变化检测算法来提高性能。下面通过显示联系人的列表看看有无管道时Angular是如何优化性能的。
...
```
运行结果是添加的`李`姓联系人并不会实时显示在页面。为什么结果不是想象中的结果呢？因为Angular使用了一个忽略变更列表的变化检测算法。例子中使用`this.contacts.push(contact);`新增一个联系人。将新联系人对象 contact 推入到数组里面，数组的对象引用并没有发生改变，还是同一个数组，所以从Angular角度来说，相同数组、没有变化，不进入`SelectContactPipe`筛选管道，数据没有更新，页面无法实时显示更新的联系人。
...
```
此例子添加pure: false 定义Impure管道，数据添加继续使用`this.contacts.push(contact);`新增将联系人推入数组中，在页面输入`李`姓联系人，回车后，也能实时显示新添加的`李`姓联系人。因为此时的管道是Impure管道，在变化周期内都会去检测执行`SelectContactPipe`管道的“transform”方法，因此数据会同步到模板视图中。
...
####Pure管道和pure 函数
在本章前面见过的管道都是用pure函数实现的。pure函数是指在处理输入并返回结果时，不会产生任何副作用的函数。 Pure管道使用Pure函数，给定相同的输入，它们总是返回相同的输出。一个pure管道必须总是用pure函数实现。
