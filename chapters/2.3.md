...
```
此时，可以通过contactForm来控制整个表单及表单组件的有效性，追踪表单组件的状态，contactForm的详细应用，下文讲解。
...
#####select 下拉框
select实现下拉单项选择功能，现构建兴趣爱好下拉选择的数据
...
使用`ngModelGroup`指令，可以方便我们在语义上区分不同性质的输入。例如姓名拆分成姓及名字，地址拆分成城市、区、街等。
...
```
`ngModelGroup`实例对象的valid可单独校验`ngModelGroup`下的所有`ngModel`是否合法。如firstname输入合法，lastname输入不合法，局部变量#nameGroup.valid为false，firstname、lastname输入都合法，局部变量#nameGroup.valid为true。
...
在学习自定义校验前，先学习如果使用模型驱动模式构建表customName单，引入`FormBuilder`
customForm
```
...
```
此时如果两个校验同时不满足，customName.errors为：
...
补充说明，在上面的例子中定义局部变量引用`NgngModel`指令#localName

在模型驱动构建的表单中，可以在创建customForm控件后,在组件上添加`control`实例对象：

```
  this.customNameControl = this.customForm.controls['customName']; 
```

需要注意的是this.customNameControl为`control`实例对象，#localName为`NgngModel`指令实例，但与#localName一样，可以在模版上使用valid进行校验错误提示。#localName获取对应的`control`实例对象可通过#localName.control。
...
1.表单内置指令`ngForm` 、`ngModel` 、`ngModelGroup` 、`ngSubmit`

* `ngForm`angular2遇到form标签会自动创建一个ngForm指令并且关联到对应的form标签上。
* `ngModelGroup`可对表单进行内容分类，ngModelGroup实例对象的valid可单独校验ngModelGroup下的所有表单是否合法。
* `ngSubmit`当表单的提交按钮type为submit时，提交按钮被点击,会触发form标签中添加的NgSubmit事件，并执行绑定到组件的onSubmit方法。
* `ngModel`表单双向数据绑定通过`[()]`符号声明，能跟踪表单的状态及校验表单值的合法性；

3.表单状态跟踪及校验

表单加入ngModel及name属性后，可以根据valid、pristine、dirty、touched、untouched等值追踪表单的状态。

4.用户自定义calss

表单加入ngModel及name属性后，用户对表单的操作状态，同时会通过表单状态类体现出来。可以通过ng-touched、ng-untouched、ng-dirty、ng-pristine、ng-valid、ng-invalid等表单状态类，自定义表单在不同状态下的样式。

5.表单验证

angular2支持的内置验证required、minlength、maxlength、pattern，使用内置校验，需在form标签中添加`novalidate`属性，屏蔽原生html5的校验，表单提交时，可以通过valid检测校验合法性。

6.表单自定义校验
自定义校验方法validateFunction，通过`FormBuilder`构建表单，并在构建control时将检验方法传入。如果想添加多个校验，可以使用Validators.compose。
...
bootstrap(AppComponent, [{provide: PLATFORM_PIPES, useValue: [DateReform], multi:true}]);
```
注入管道`DateReform`,此时管道`DateReform`为全局自定义管道，像内置管道一样可以在任何组件中随意使用，且无需在定义组件前import `DateReform`管道，也无需在`@Component`装饰器的pipes数组里列出使用`DateReform`管道等，节省许多重复操作，大大提高开放效率，且代码简洁多了。
...
Angular2提供许多内置管道，如日期格式化管道`DatePipe`、文本转换大写管道`UpperCasePipe `、文本转换小写管道`LowerCasePipe`、数值格式化管道`DecimalPipe`、货币格式化管道`CurrencyPipe`、百分比格式化管道`PercentPipe`、文本替换管道`ReplacePipe`、数组或字符串裁剪管道`SlicePipe`、异步管道`AsyncPipe`、Json格式化管道`JsonPipe`。

4.自定义管道

angular提供管道的原型，通过继承管道原型及重写原型的transform方法可以自定义管道，使用自定义管道，需在@Component装饰器中声明`pipes: [pipeName]`,此时，管道可以在该组件模版中局部范围内使用。
同时angular提供自定义全局管道，可以使自定义管道与内置管道一样，无需在@Component装饰器中声明，也可以在所有的组件模版中使用。
