...
数据流向 | 语法 | 绑定类型
------ | ------ | ------
单向：从数据源到视图目标（属性绑定） | ```{{expression}}<br>[target] = "expression"<br>bind-target = "expression"``` | DOM对象属性(property)绑定<br>HTML标签特性(attribute)绑定<br>CSS类绑定<br>Style样式绑定
单向：从视图目标到数据源（事件绑定） | ```(target) = "statement" on-target = "statement"``` | 事件
...
属性绑定实现了这种实现了数据从组件流向视图，事件绑定则实现了数据从视图流向组件，两者合二为一，双向绑定就诞生了。但这样看起来很繁琐，谁能记住哪个元素属性用于设置，哪个用于汇报用户更改？又该如何从输入框中提取出当前显示的文本，以便更新数据属性？谁想每次都去查一遍？使用Angular里的NgModel指令可以更便捷的进行双向绑定。
...
上面可以看出，`[(ngModel)]`可以拆分为两部分`ngModel`和`ngModelChange`两部分组成，实际上，`ngModel`是作为`ngModel`指令的输入属性用来设置元素的值，`ngModelChange`作为`ngModel`指令的输出属性用来监听元素值的变化。也就是说当Angular看到一个`[(x)]`的绑定目标时，会期待这个`x`指令有一个名为`x`的输入属性和一个名为`xChange`的输出属性。即

```
[(x)] = "e" <==> [x]="e" (xChange)="e=$event"
```

注意这里，`e=$event`。习惯性会认为`$event`变量是来自`DOM`事件。但` ngModelChange`属性并不会生成`DOM`事件——实际上它是一个`Angular EventEmitter`类型的属性。
所以实际上`[(ngModel)]`是的具体实现如下，不过需要一些本书后面的指令相关的知识。
...
总结一下，简单的说双向绑定实际上就是通过输入属性存储数据，同时通过一个相对的输出属性（输入属性+change后缀）来监听输入属性存储的数据变化并触发对应的事件。
有了这些知识，接下来扩展一下，创建一个支持双向绑定的组件。此时在组件中需要定义一个`@Output`输出属性来匹配一个`@Input`输入属性。
...
export class Amount {
  @Input() number: n = 0;
  @Output() numberChange: EventEmitter<n> = new EventEmitter<n>();
...
@Component({
	...
	template: `
    <div>
      <amount [(number)]="number1">Number 1:</amount>
	    <amount [number]="number2" (numberChange)="number2=$event">Number 2:</amount>
	    
      <ul>
      	<li> Number 1: {{number1}} </li>
      	<li> Number 2: {{number2}} </li>      
      </ul>
	  </div>
	`,
	directives: [Amount]
...
##3.5 表单
前文模板部分讲解了如何使用模板完成数据的展示，本节继续讲解如何使用表单作为数据采集的入口`表单`。

表单作为数据采集的重要入口，几乎是所有WEB应用必不可少的一部分，使用场景很广泛，常见的场景有新用户注册，用户登陆、各类数据的添加修改、问卷调查、文件上传等等。HTMLHTML提供了许多表单标签如文本、密码、单选框、复选框等，且内置简单校验，但是内置校验提示效果在各个浏览器表现都不同，校验错误提示文字各异，无法统一，且与应用本身的UI风格相差甚远，其次，自定义校验规则、表单数据获取、处理、提交等流程异常复杂。

针对上述问题，Angular团队在Angular中对表单进行了封装扩展，为校验、提示、自定义UI、数据处理等问题提供了很好的解决方案。Angular模板表单能使用双向绑定数据、设置校验规则、自定义校验错误提示样式效果、根据输入结果开启禁用表单等等。通过Angular表单，可以使用简洁的代码、灵活接口，构建丰富交互体验、强用户引导的表单。
...
###3.5.1 模板中的表单
模板驱动方式构建表单以模板为主，大部分表单功能可以在模板中完成。下面，先通过添加新联系人表单来简单的了解模板中的表单，表单效果如图2.3-1。
...
###3.5.2 表单指令
表单指令是Angular对常用的表单交互功能进行封装扩展，负责处理大多数绑定数据、指定校验规则、显示校验错误等重复单调的任务，使我们能在模板中快速构建交互友好的表单。接下来会一一讲解各个表单指令的功能及指令引申出的相关知识点，涉及的内容如下图所示：

![MacDown logo](https://raw.githubusercontent.com/gf-rd/gf-angular2-book/master/_images/chapters2-3/form-directives.png)
...
```
代码中并未出现`NgForm`相关的指令，为什么说已经使用了`NgForm `指令？因为通过`providers`数组参数注册了表单模块后，Angular模板在编译解析时，遇到`form`标签会自动创建一个`NgForm`指令并且关联到对应的`form`标签上。使用`NgForm`指令对象，可以通过＃符号定义表单局部变量contactForm，并将contactForm变量初始化为`ngFrom`，如下：
...
>在表单中使用`ngModel`，必需给控件添加name属性(如name="name")，否则会报错。因为`NgForm`指令会为表单建立一个控件组对象`FormControls`，作为表单控件的容器。控件使用`ngModel`会以name为唯一标识符来注册生成一个`FormControl`，并加入`FormControls`容器中。
...
当Angular遇到`[(ngModel)]`时会翻译成一个`ngModel`的输入属性和一个`ngModelChange`的输出属性，自动地完成了属性绑定和事件绑定。因此，在表单中也可以使用下面的方法实现双向绑定，效果是一样的。

``` html
<input type="text" name="name" [ngModel]="curContact.name" (ngModelChange)="curContact.name = $event">
```
这里值得注意的是`curContact.name = $event`，以前看到的`$event`变量是来自 DOM 事件的， 但 ngModelChange 是一个 Angular EventEmitter 类型的属性，不会生成 DOM 事件。当`ngModelChange`触发时会执行EventEmitter的emit方法，$event为emit方法传入的参数，这里默认为输入控件的值，因此`[ngModel]="curContact.name" (ngModelChange)="curContact.name = $event"`也能实现双向数据绑定。
...
>值得注意的是，经测试，多选下拉框在pc chrome中，选择需按住鼠标才能选择多个。
...
####局部模板变量
局部模板变量（Template reference variables）下面统称局部变量，是模板中对DOM元素或指令的引用，能在DOM元素中使用，也能用于Angular组件。局部模板变量可以使用在同一元素、兄弟元素或任何子元素中。

##### 定义局部变量引用DOM元素
若将元素定义为引用DOM元素局部变量，只需在其前面加上`#`符号，或者`ref-`前缀。如：

```HTML
<li class="form-group" >
  <label for="name">姓名：</label>
  <input type="text" #name name="name" id="name">
  <p>{{telNum.className}}</p>
</li>
<li class="form-group" >
  <label for="telNum">电话：</label>
  <input type="text" ref-telNum name="telNum">
   <p>{{telNum.className}}</p>
</li>
```
Angular会自动把变量的值设置为当前元素的DOM对象的引用，既`document.getElementById("name")`对象的引用。定义局部变量使我们能直接在模板中使用元素的DOM属性，如例子中的`telNum.className`及`telNum.className`。

##### 表单中的局部模板变量
表单中也可以定义局部变量，模板表单中的局部变量与引用DOM元素的局部变量有些区别。引用DOM元素的局部变量通过#定义，默认赋值为当前元素DOM对象的引用。模板表单中的局部变量在定义时会被赋值为表单指令对象的引用，能追踪表单的状态及有效性。
...
```
当往姓名控件输入`李四`，电话控件输入`12345678901`，获取contactForm.value的值如下所示：
...
```
contactForm.value属性是一个简单的JSON对象，键对应控件元素的`name`属性值，值对应于控件元素的`value`值。 对于简单的AJAX请求提交表单采集的数据，这无疑是令人兴奋的功能，直接将表单局部变量contactForm的value提交即可，不需要自己构建JSON对象，非常简洁方便。

表单控件也可以定义局部变量，以姓名控件为例，添加自定义局部变量：

```html
<input type="text" name="name" [(ngModel)]="curContact.name"  #name="ngModel">
<p>{{name.valid}}</p>
```
此处定义了局部变量name初始化为`NgModel`，并通过name.valid获取了控件的合法性状态。在定义表单局部变量contactForm时初始化为`ngForm`，两者的区别是表单局部变量contactForm为`NgForm`指令，是`NgForm`实例对象的引用，控件局部变量name为`ngModel`指令，是`ngModel`实例对象的引用。
...
下面，以姓名控件为例，给控件添加required属性，对姓名控件进行获取焦点、输入、删除输入等操作，观察控件每一步操作的状态变化。控件状态变化如图2.3-3:

![MacDown logo](https://raw.githubusercontent.com/gf-rd/gf-angular2-book/master/_images/chapters2-3/form-input-status.png)
<br>图 2.3-3<br>
...
```
例子中，绑定了`ngSubmit`事件，`ngSubmit`为EventEmitter事件类型。当提交按钮被点击，首先执行表单原始的`onSubmit`事件，`onSubmit`事件中触发绑定的`ngSubmit`，执行`FormComponent`组件中定义的`onSubmit`方法，并contactForm.value的值作为参数传入，并对传入的数据进行处理及上传。

代码中添加了`[disabled]="!contactForm.valid"`，绑定提交按钮的`disabled`属性，当输入表单值全部检验通过时contactForm.valid为true。相反，如果有其中一个或多个表单值校验不通过，contactForm.valid为false，添加按钮不可用，避免了许多无效的点击和请求，减轻了服务器的负载。

代码中添加类型为“reset”的按钮，该按钮实现了angular表单重置的功能。点击该按钮，首先执行表单原始的`onReset`事件，`onReset`事件中触发相关表单内容的重置。
...
控件css类名根据控件状态变化而变化，具体可见2.3-3控件状态变化图。使用代表控件状态的css类名，我们可以自定义css类名中的样式，实现自定义跟踪状态的效果，如：
...
####自定义校验
Angular提供的内置表单校验基本能满足大部分校验场景，倘若需要实现复杂的表单校验，可以使用Angular提供的自定义校验。下文将从一个例子一步一步讲解表单自定义校验。
...
@Component({
  ...
  <div class="container">
	 <form [ngFormModel]="customForm" (ngSubmit)="onSubmit(customForm.value)" novalidate>
		 <li class="form-group" >
		  <label for="name">姓名</label>
		  <input type="text" name="customName"  [(ngModel)]="customName">
		 </li>
		 <li class="form-group" >
		  <label for="tel">电话</label>
		  <input type="text" name="customTel"  [(ngModel)]="customTel">
		 </li>
		 ...
	</form>
  </div>
...
```
该例子使用模型驱动方式构建表单，自定义构建了`FormGroup`表单实例变量customForm和`control`指令对象customName和customTel。

在装饰器`@Component`添加`directives: [REACTIVE_FORM_DIRECTIVES]`后，绑定`ngFormModel`属性来使用已经存在的`NgForm`指令对象。在表单控件中`name`、`ngModel`属性都赋值为组件中定义的`Control`指令对象名customName，来使用已经存在的`Control`指令对象。
...
`Control`指令对象`customName`的值为数组，数组索引0的值为控件初始值，索引1的值为该控件的校验配置。customTel添加的是`Validators`内置校验`Validators.required`，与直接在表单元控件素中添加`required`属性功能效果一致。customName添加的是自定义校验`validateUserName`，`validateUserName`函数定义了自定义用户名的校验规则：用户名必需是邮件格式或者是手机号码格式。若表单值不满足该用户名的校验规则，则返回userName的错误信息，满足返回null。

通过上文的学习，我们知道如果控件校验不通过时，控件对应的`Control`实例对象的属性errors会返回校验不合法的对象信息。在自定义校验中同样适用，如下所示：
...
需要注意的是自定义的customName为`control`实例对象，而控件局部变量localName为`NgModel`指令实例，localName获取对应的`control`实例对象可通过localName.control获取。
...
```
至此，表单的指令、状态追踪、自定义校验等内容已经讲述完毕，下面开始学习新的模板内容－管道。

##3.6 管道（pipes）
管道是用于将模板内的数据转换成用户能快速识别的数据来显示。如日期数据，对于用户来说，只需要看日期信息，而日期的原始数据为Mon Jun 06 2016 14:17:00 GMT+0800 (CST)，这样的展示不直观，用户更喜欢直接看到转换后的日期信息2016-06-06。Angular管道可以很优雅地在模板中执行一个管道函数获取其返回值，使得代码更清晰，更有层次。本小节将学习管道的基本用法、Angular提供的内置管道、如何自定义管道以及pure管道和impure管道的区别。
...
```
经过JSON管道转换的，在视图中显示的是对象`JSON.stringify`后的值，在调试时可以一目了然查看对象内部变量和值，有助于快速定位问题。
...
@Component({
  ...
  template: `
   <div>
    <p>e变量: {{e | number:'3.4-5'}}</p>
    <p>pi变量: {{pi | number:'3.1-5'}}</p>
  </div>
  `，
})
export class AppComponent { 
...
pi变量: 33，456.0
```
对于变量e进行数值本地格式化的参数为“3.4-5”，参数`.`之前的3表示整数位最少保留3位，原数值整数位为1位，不足3位，用前导0填充，填充后整数位为“002”。参数`.`后面的`4-5`表示最小保留分数位为4位，最大保留分数位为5位，因原始数据分数位大于5位，最大保留5位限制发挥作用，经四舍五入后分数位为“71828”。
...
B变量: $0，001.35
```
例子中，变量a第一个参数为USD，表示转换成美元货币格式，第二个参数为false，货币前缀展示美元`ISO 4217`货币码。变量b第二个参数为true，货币前缀展示美元货币符号，第三个参数表示对货币数值进行数值本地化，因此解析后转换成货币格式为“$0，001.35”。
...
B变量: 0，134.950%
```
例子中，变量a直接转换成百分比格式。变量b首先经过百分比将小数点往右移动两位后变成134.95，然后进行数值本地化，因此解析后转换成百分比格式为“0，134.950%”。
...
```
`SlicePipe`以JavaScript API`Array.prototype.slice()`及`String.prototype.slice()`的功能效果为基础，在模板管道中实现相同的功能。输入值expression可为数组或者字符串，参数start和end为数组的索引，具体定义可参考JavaScript API`Array.prototype.slice()`及`String.prototype.slice()`的介绍，这里不赘述。
...
``` 
`ReplacePipe`以JavaScript API`String.prototype.replace()`的功能效果为基础，实现了类似的功能，参数pattern和replacement的具体定义可参考JavaScript API`Array.prototype.replace()`的介绍，这里不赘述。

### 3.6.2 自定义管道
虽然Angular提供了许多内置的管道指令，但是数据转换的格式各式各样，内置管道不能一一满足需求。Angular提供自定义管道功能，可根据业务场景自定义各式各样的数据转换需求。下面通过定义一个日期格式化的自定义管道来认识模板自定义管道。

#### 定义元数据
在使用`@Pipe`前必须从`@Angular/core`中引入`Pipe`组件。
...
```
例子中定义了一个名为`DateReform`的管道类，`DateReform`管道类继承类`PipeTransform`，`DateReform`管道`transform`方法接收一个需要转换的日期输入参数val，及一个可选代表日期格式的参数format，根据日期格式，将输入的日期进行格式话，最后返回转换后的日期字符串值。
...
```
在模板中使用管道，需组件`@Component`装饰器声明`pipes: [DateReform]`，告诉模板可以使用自定义管道`DateReform`，之后可以在模板中随意使用管道`DateReform`。
...
定义全局自定义管道需在根组件中注入自定义管道的服务。在启动应用入口页面引入自定义管道`DateReform`，同时从`'@Angular/core'`引入`provide`、`PLATFORM_PIPES`组件。
...
```
使用`{provide: PLATFORM_PIPES， useValue: [DateReform]， multi:true}`注册日期格式提供器，在根组件上依赖注入`DateReform`提供器服务，它会在整个应用程序中被共享，详细关于提供器的内容，会在依赖注入章节讲解。<br>
此时管道`DateReform`为全局自定义管道，像内置管道一样可以在任何组件中随意使用。

### 3.6.3 管道状态
Angular在每次点击、移动鼠标、定时器触发、服务器响应等事件后会执行数据绑定值变化检测，这会带来性能问题，因此如果我们使用管道，Angular会选择使用更简单、更快速的变化检测算法来提高性能。下面通过显示过滤的联系人列表看看有管道时Angular是如何优化性能的。
...
```
上述例子中，定义了一个过滤'李'姓联系人的管道`SelectContactPipe`，联系人的列表经过管道`SelectContactPipe`的过滤，只显示'李'姓联系人。在表单输入一个'李'姓联系人后，回车触发addContact，通过push将新联系人推入数组中。按照代码的逻辑，此时应该会在联系人列表中实时显示新'李'姓联系人，但结果并不非如此。

因为Angular使用了一个忽略变更列表内部数据变化的检测算法对性能进行了优化。例子中使用`this.contacts.push(contact)`新增一个联系人，数组对象引用没有发生改变。所以从Angular角度来说，相同数组，不进入`SelectContactPipe`筛选管道，列表数据没有更新，页面不会实时显示更新的联系人。这个筛选管道是pure管道。
...
```
在上述过滤`李`姓联系人的例子中，给管道添加pure: false 定义成Impure管道。Impure管道在变化周期内都会去检测执行`selectContact`管道的“transform”方法，对变化后的联系人列表进行过滤，因此联系人列表数据会同步到模板视图中。
