...
同时子类里可以重写父类的方法，MotoCar和Jeep的drive方法重写了Car的drive方法，这样drive方法根据不同的类而具有不同的功能。 注意，即使landRover被声明为Car类型，它依然是个Jeep，landRover.drive(200)调用Jeep里的重写方法;包含constructor函数的派生类必须调用super()，它会执行基类的构造方法。
...
从上面的例子可以看到，在函数和返回值类型之间使用（=>）符号，使函数便于理解。函数的返回类型是必须的，如果函数没有返回值，必须置顶返回类型为void。
...
TypeScript还提供了默认初始化的参数，当调用者没给这个参数传值或者传的值为undefined时，此时参数的值就为设置的默认值，假如我们给上面例子的lastName设置初始化值'Brown',例如：
	```typescript
...
##1.3.5 模块
模块化开发已经成为近些年来前端代码组织的大趋势，AMD，Commonjs等模块化系统已经使用了好长时间，之后browserfy，webpack等打包方案把模块化开发推向了新的高度，从ECMAScript 2015开始引入模块的概念，Typescript也沿用了这个概念， “内部模块”称做“命名空间”， “外部模块”简称为“模块”。一般提到的模块即是”外部模块“。
...
``` typescript
export * from "./IdentityValidate"; // 导出接口
...
//Consumer.ts
	import * as animal from "./MyLargeModule.ts";
	let dog = new animal.Dog();
```
* 使用重新导出进行扩展: 我们可能经常需要去扩展一个模块的功能，推荐的方案是 不要去改变原来的对象，而是导出一个新的实体来提供新的功能。

``` ts
import $ from "./Jquery"

$.fn.myExtend = function() {
	// logic
	return this;
}
export { $ as Jquery } ;
...
### 内部模块
与外部模块对应的是内部模块,内部模块现在也称为`namespace`,创建一个封闭的作用域，通过一个公有的前缀导出给外部使用,避免全局对象命名污染,在大型的项目中内部模块通常封装一个独立完整的模块功能供外部调用。

``` typescript
namespace Validation {
	const COMPANY = "广发证券"; //导出变量
...
let validate = new Validation.ErpIdentityValide(),
	  erp = 27012;
console.log(validate.isGfStaff(erp));
```
命名空间在大型系统中能明确各模块的职责，有利于项目的开发，维护，扩展。

### 三斜线指令
除了上面的import方式，typescript中还支持另一种加载命名空间，即`三斜线指令`，书写方式如下：
```/// <reference path="Validation.ts" />```
三斜线指令是包含单个XML标签的单行注释，注释的内容会做为编译器指令使用，有几点要注意
* 三斜线指令仅可放在包含它的文件的最顶端。 
* 一个三斜线指令的前面只能出现单行或多行注释，这包括其它的三斜线指令。
* 如果它们出现在一个语句或声明之后，那么它们会被当做普通的单行注释，并且不具有特殊的涵义。
三斜线用于声明文件间的依赖，告诉编译器在编译过程中要引入的额外的文件，类似html中的script标签，对于引用的顺序同样有要求。
...
`装饰器（Decorators）`装饰器是一种特殊类型的声明，它能够被附加到类声明，方法，访问符，属性，或参数上。 装饰器利，形如`@expression`，expression求值后必须为一个函数，它使用被装饰的声明信息在运行时被调用。
