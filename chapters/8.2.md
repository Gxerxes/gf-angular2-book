...
接着，将依次出现一些问题，询问用户相关的配置项，每个问题均提供了一个建议的配置，也可以按照项目需求输入。如下图所示，第一个问题询问了要使用的测试框架，默认的配置为jasmine，按enter将选择jasmine作为测试框架，也可以按tab切换其它的配置可选值。简单起见，可以均使用默认配置，也就是一路按enter键，最终，将在当前所在目录生成karma.config.js文件
