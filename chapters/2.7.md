...
通过provideRouter方法获取的angular默认路由服务之一，为默认的路由策略PathLocationStrategy。使用该策略的应用，需要为其定义一个base路径，用于作为前缀来生成和解析URL（路由策略的详细内容将会在某小节中提到，在此先行略过）。Angular提供了两种方式来对base路径进行设置：
