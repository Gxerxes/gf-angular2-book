...
此时在浏览器上输入http://localhost/component-a或者http://localhost/component-b，还不会创建对应Component的实例，自然也就还不会在页面上显示个Component对应的内容。
...
### 路由显示
要想显示Component，需要在路由组件的模版中使用<router-outlet>指令。相当于一个占位节点，在模版中开辟出一块区域，每个路由所对应的Component都将在这个区域内显示出来。一个模版只能定义一个匿名的<router-outlet>元素，但是可以定义任意多个命名的<router-outlet>元素。详细内容会在从路由部分深入展开。
