...
在编辑页面，会初始化好友的信息，并可以在上面修改，如果修改的内容格式不对，会在输入框右边显示一个红色的竖线提示。表单下方有保存和取消按钮。
...
新建一个tsconfig.json文件，TypeScript编译配置文件，通过一些设置来指导Typesript进行编译，放于项目根目录下，并输入以下配置
```
{
  "compilerOptions": {
    "module": "commonjs",
...
```
"module"：指定模块代码生成。"target":ECMAScript指定版本。
