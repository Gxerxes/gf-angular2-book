...
```
在这个文件内容中，我们首先通过import从Angular的基础包core中引入组件（Component），通过组件我们可以定义该组件的dom元素名称，如：selector: 'seed-app'，也可以给组件引入所需要的模板，如：templateUrl: 'seed-app.html'。最后，通过类的形式定义该组件对外输出的类名，这样在其他的文件中就可以通过这个类名引用该组件。关于组件的具体内容可以在2.2章节中深入了解学习。
...
从上面的文件中，可以看出每个文件都有依赖一些特定的包，为了能是每个文件引入的模块生效我们需要要引入Angular的基础包，这样才能驱动整个项目的运行，这里我们创建一个vendor.ts的文件放在根目录，它里面引入整个项目依赖的Angular基础包，内容如下：
```
