...
```
NgModule是用来传人元数据，装饰器函数中declarations是声明项目的组件；imports用来导出当前模块所需的“素材”，几乎每个应用的根模块 都要导入 BrowserModule 模块；bootstrap 标记出根组件，在启动应用时，Angular应该通过它来进行引导。
...
1.在对应的项目根目录终端中，运行npm install命令，会根据package.json配置文件，安装所依赖的包。安装的过程中，会自动在根目录上创建一个node_modules文件夹，项目依赖的包都在其中。该过程中，有的包安装可能会下载失败，原因可能是连接不到外网，解决方法：使用淘宝的cnpm，或者是公司的镜像等<br/>
