...
PS: 会根据 Angular 新的进展来小幅更新后续发展和历史展望
...
- 介绍通讯录例子有哪些内容
- 项目预览图介绍各大模块的用途
- 项目中一些通用概念的介绍，各大模块之间的联系介绍

##### 1.4.2 准备工作
- 使用的工具流介绍 （各类工具介绍）
- 如何进行安装使用相关工具
- 环境搭建（此处分mac与windows两种环境的安装教程）

$================= 第一作者 2016-06-13 =================  

##### 1.4.3 Hello World 项目
- 基于上面搭建好的环境编写一个最简单的hello world出来
- 介绍项目的整体目录布局
- 一步步创建与输入相关命令的反馈介绍
- 依赖的一些库的介绍
- 把整个例子跑起来
...
- 概述（overview）
    - 引出模板的概念和特点【建议参考官网模板的引文】，并说明模板在angular2中的应用场景和实际意义
    - 介绍表单及其应用场景，对将要书写的核心内容做个引出
    - 介绍pipe在模板中的目的和给用户体验的意义
...
- overview 
    - 由模板引出本节要书写的内容
    - 设计一个demo需求，能够贯串本节所有内容

##### 3.5.1 模板中的表单（Template-Driven Forms）
- 根据quickstart demo拓展一个表单例子
- 根据设计的demo围绕表单组件和表单模板展开，要从具体的代码来展开【可参考官网】
    
##### 3.5.2 表单指令  用上节的例子来继续拓展，引出一下的表单指令
- NgForm - 指令依赖声明 & 局部变量
- NgControl - 从NgControl的运用来讲 包括Track change-state & validity
- NgModel - NgModel的运用、NgModel双向绑定原理不需要再继续深入，袁野在双向绑定会深入讲解 
- NgSumit

$================= 2016-06-06 =================

- 时间点交付：设计好demo需求，完成demo的书写，完成表单指令
...
- http://www.hubwiz.com/class/567a414e660c92d638a68bf3
...
- 管道参数 
    - 什么是管道参数，如何使用
    - 如何给组件绑定format parameter
- 链式管道 
    - 给代码例子讲解即可
- 管道的内置方法 管道的内置方法怎么使用，列举以下的内置指令
    - DatePipe
    - JsonPipe
    - UpperCasePipe
    - LowerCasePipe
...
##### 3.6.3 管道状态 （Pure and Impure Pipes，需和@邓玉龙确定该节词汇怎么翻译）
- 引出Pure and Impure Pipes
- Pure Pipes
    - 讲述在什么情况下触发Pure Pipe
    - 什么是pure Pipes
- Impure Pipes
    - 讲述什么是Impure Pipes，通过代码示例来讲解
...
##### 5.1.2 NG2中的Service（Injectable）  （1000字内）
...
#### 6.4 Angular2中如何使用依赖注入  

##### 6.4.1 组件注入Service：结合通讯录实现一个简单的组件依赖注入demo，参照官网的实现步骤 
...
##### 6.4.2 Service注入Service: Service依赖另一个service时候如何实现注入，可参考官网结合通讯录的场景设计一个demo!


##### 6.4.3 介绍optional injection：Ng2中如何实现可选的依赖注入  
$=================第二作者: 2016-07-1 =================
...
- 用户管理组件：主要介绍ng2 form的知识，包括验证，绑定等，同时完善后台代码中的用户管理部分。
